...
import static yvs.dao.salaire.service.Constantes.GRH_DUREE_POINTAGE_REPOS;
import yvs.dao.salaire.service.Options;
...
import static yvs.grh.paie.evalExpression.UniteLexicale.calcul;
import static yvs.grh.paie.evalExpression.UniteLexicale.calculLogique;
import static yvs.grh.paie.evalExpression.UniteLexicale.isANumber;
...
//    int i = 0;
//
//    public double calculQuantite(String exp, String codeElt) {
//        i = 0;
//        List<Lexemes> tab = buildLexemeToLexeme(exp, codeElt);
//        if (tab != null) {
//            String formule = rewriteExpression(tab);
...
//                try {
//                    Double re = Double.valueOf(formule);
//                    return re;
//                } catch (NumberFormatException ex) {
//                    formule += " #";
//                    formule = evalueExpression(convertECPToEPOST(formule.trim()));
//                    if (!formule.trim().isEmpty()) {
//                        return (Double.valueOf(formule));
//                    } else {
//                        erreurs.add(new Options(exp, " la variable " + exp + " n'a pas pu être évalué !"));
//                    }
//                }
//
//            }
...
//    }
    ...
    private double buildCategorie(List<YvsGrhElementSalaire> list) {
        double result = 0;
        System.err.println("-- cONTAINS = " + listEltSalaire.size());
        for (YvsGrhElementSalaire lx : list) {
            if (listEltSalaire.contains(lx)) {
                System.err.println("-- Catégorie = " + lx.getCode());
    ...
    public String findVar(Lexemes lxVar) {
        ...
        if ((re = findVarInListTempon(lxVar.code)) != null) {
            ...
            if (lxVar.code.startsWith("categorie")) {
                List<YvsGrhElementSalaire> lcat = dao.loadNameQueries("YvsGrhElementSalaire.findByCategorie", new String[]{"categorie", "societe"}, new Object[]{UtilExpression.extract(lxVar.code, "categorie."), currentScte});
                double r = buildCategorie(lcat);
                return " " + r;

            } else if (lxVar.code.startsWith("employe")) {
                //évalue les éléments de salaire disponible sur la fiche employé                 
                re = " " + calculEltSalaire(lxVar.code, currentEmps, contrat);
            } else if (lxVar.code.startsWith("bulletin")) {
                String[] part = split(lxVar.code);
                for (String s : part) {
                    //System.err.println("--- Parti = " + s);
                }
                if (part.length == 3) {
                ...
                re = " " + calculeNbJourValideByCode(yvs.dao.salaire.service.UtilExpression.extract(lxVar.code, "pointage."));
                ...
                re = " " + findTauxValidationPointage(yvs.dao.salaire.service.UtilExpression.extract(lxVar.code, "tauxP."));
            } else {
...
//                    getMessage("Impossible de terminer. Votre expression est incorrecte", FacesMessage.SEVERITY_ERROR);
    ...
    private double calculEltSalaire(String code, YvsGrhEmployes currentEmps, YvsGrhContratEmps contrat) {
        ...
        switch (code) {
            case Constantes.S_JOUR_DE_CONGE:
                result = getDureeCongeRestant(currentEmps.getId());
                break;
    ...
    private double calculEltMission(String code, YvsGrhContratEmps contrat) {
        double result = 0;
        switch (code) {
            case yvs.dao.salaire.service.Constantes.GRH_DUREE_MISSION:
...
//        return (Double) dao.getNbreJourConge(idEmp, fin);
    ...
    private double getHeureSup(long idEmp) {
        String requete = "SELECT SUM(y.total_Heure_Sup) FROM Yvs_Grh_Presence y WHERE y.valider=true AND y.employe=? AND (y.date_Debut BETWEEN ? AND ?)";
        yvs.dao.Options param_[] = new yvs.dao.Options[]{new yvs.dao.Options(idEmp, 1), new yvs.dao.Options(debutTraitement, 2), new yvs.dao.Options(finTraitement, 3)};
//        Double d = (Double) dao.loadObjectByNameQueries("YvsGrhPresence.countTotalHS", new String[]{"employe", "date1", "date2"}, new Object[]{new YvsGrhEmployes(idEmp), debut, fin});
    ...
    private double findDureeMission(YvsGrhEmployes e) {
        String requete = "select public.jour_mission_autorise(?,?,?)";
        yvs.dao.Options param_[] = new yvs.dao.Options[]{new yvs.dao.Options(e.getId(), 1), new yvs.dao.Options(debutTraitement, 2), new yvs.dao.Options(finTraitement, 3)};
        return (Double) dao.callFonction(requete, param_);
    }
    ...
    private double getHeureSupDimanche(long idEmp) {
        ...
        String requete = "SELECT SUM(y.total_Heure_Sup) FROM Yvs_Grh_Presence y WHERE y.valider=true AND y.employe=? AND (y.date_debut BETWEEN ? AND ?) AND extract (DOW FROM y.date_debut)=0";
        yvs.dao.Options param_[] = new yvs.dao.Options[]{new yvs.dao.Options(idEmp, 1), new yvs.dao.Options(debutTraitement, 2), new yvs.dao.Options(finTraitement, 3)};
        return (Double) dao.callFonction(requete, param_);
    }
    ...
    private double getHeureSupJFerie(long idEmp) {
        ...
        String requete = "SELECT SUM(y.total_Heure_Sup) FROM Yvs_Grh_Presence y WHERE y.employe=? AND y.valider=true AND y.date_debut IN "
                ...
                + "OR ((js.jour BETWEEN ? AND ?) AND js.all_year is false)))";
    ...
    private double giveMontantRetenue(String codeRetenue) {
        ...
        val = new Object[]{contrat, fin};
        double r = 0;
        List<YvsGrhDetailPrelevementEmps> lpe = dao.loadNameQueries("YvsGrhDetailPrelevementEmps.findByDatePrelevement", champ, val);
        for (YvsGrhDetailPrelevementEmps d : lpe) {
    ...
    private int giveNbEnfant(YvsGrhContratEmps ce) {
        {
            ...
            int r = 0;
            List<YvsGrhPersonneEnCharge> list = dao.loadNameQueries("YvsGrhPersonneEnCharge.findByEmploye", new String[]{"employe"}, new Object[]{ce.getEmploye()});
            for (YvsGrhPersonneEnCharge p : list) {
    ...
    private String[] split(String code) {
        System.err.println("A Split --- " + code);
    ...
    private double givePrime(String code) {
        ...
        for (YvsGrhElementAdditionel e : contrat.getPrimes()) {
            if (e.getTypeElement().getCodeElement().equals(code)) {
                //si la prime fait l'objets de suspension
                if (!e.getRetenues().isEmpty()) {
                    //vérifions que la date de suspension est incluse dans la période de calcul
                    for (YvsGrhDetailPrelevementEmps p : e.getRetenues()) {
                        if (((p.getDatePrelevement().after(debutTraitement) && p.getDatePrelevement().before(finTraitement)) || p.getDatePrelevement().equals(debutTraitement) || p.getDatePrelevement().equals(finTraitement)) && p.getStatutReglement() != 'S') {
                            result = result + e.getMontantElement();
                        }
                    }
                } else {
                    if (!e.getPermanent()) {
    ...
    private double findDurreConge(String code) {
        switch (code) {
            case Constantes.GRH_DUREE_CONGE_PRINCIPAL:  //déterminer le nombre de jour de congé du à un employé 
                return (int) getDureeCongeRestant(contrat.getEmploye().getId());
            ...
            case yvs.dao.salaire.service.Constantes.GRH_DUREE_CONGE_AUTORISE:
                //évalue la durée du congé autorisé entre deux date                
                return findNbDayCongeAutorise(contrat.getEmploye());
            case yvs.dao.salaire.service.Constantes.GRH_DUREE_CONGE_N_AUTORISE:
                //évalue la durée du congé autorisé entre deux date                
                return findNbDayCongeNonAutorise(contrat.getEmploye());
            case yvs.dao.salaire.service.Constantes.GRH_DUREE_CONGE_IN_MONTH:
                //évalue la durée du congé autorisé entre deux date                
                return findNbDayCongeInMonth(contrat.getEmploye());
            default:
                //cas des congé technique
                String chaine = yvs.dao.salaire.service.UtilExpression.extract(code, "conge.");
                System.err.println(chaine + "-- find Result taux ");
                if (chaine.startsWith("tauxCT")) {
                    //récupère le taux
                    System.err.println("-- find Result taux " + UtilExpression.extract(chaine, "tauxCT."));
    ...
    private long calculeNbJourValideByCode(String codeP) {
        ...
        val = new Object[]{codeP, contrat.getEmploye(), debut, fin};
    ...
    private double findCongeTechnique(boolean duree, String code) {
        ...
        if (p != null) {
            if (duree) {
                ...
                yvs.dao.Options param_[] = new yvs.dao.Options[]{new yvs.dao.Options(debut, 1), new yvs.dao.Options(fin, 2), new yvs.dao.Options(p.getNumMois(), 3), new yvs.dao.Options(contrat.getEmploye().getId(), 4)};
                re = (Integer) dao.callFonctionInt(requete, param_);
            } else {
                System.err.println("-- give Result ");
                re = p.getTaux();
            }
        }
        return re;
    }
//    private int findDureeCongePrincipal(YvsGrhEmployes e, Date jour_) {
//        int result;
//        //calcul maintenant le nombre de congé
//        String requete = "select public.conge_jour_restant(?,?)";
...
//    }
    ...
    private int findNbDayInPeriodeDeRefence(YvsGrhEmployes e) {
        String requete = "select give_nbDay_in_periode_reference.give_periode_reference(?,?)";
    ...
    private int findNbDayCongeInMonth(YvsGrhEmployes e) {
        System.err.println("Date " + Managed.dfML.format(debutTraitement));
    ...
    //récupère le nombre de jour de travail effectif
    public double giveDayOfWorkReel(long idEmp, Date debut, Date fin) {
        String requete = "select public.jour_travail_effectif(?,?,?)";
        yvs.dao.Options param_[] = new yvs.dao.Options[]{new yvs.dao.Options(idEmp, 1), new yvs.dao.Options(debut, 2), new yvs.dao.Options(fin, 3)};
        return (Double) dao.callFonction(requete, param_);
    }
    ...
    public double findLastSalaire(String numero, Date date) {
        ...
        try {
            ...
            //calcule la date du mois correspondant
            Calendar c = Calendar.getInstance();
//            c.setTime(listPlanification.get(listPlanification.indexOf(currentBulletin.getEntete())).getFinMois());
            c.setTime(date);
            c.add(Calendar.MONTH, -n);
            c.roll(Calendar.DAY_OF_MONTH, false);
    ...
    public int findNbreDayInMonth(String numero) {
        ...
        try {
            ...
            if (n > 0) {
                ...
                d = (int) giveDayOfWorkReel(contrat.getEmploye().getId(), debut, fin);
            }
        } catch (NumberFormatException ex) {
            //
            return 0;
    ...
    private double findElementSalaireInParamaBulletin(String code) {
        System.err.println(code + "  header " + header);
