...
public class PaginatorResult<T extends Serializable> {
    ...
    private String nameQueriCount, nameQueri, queryIds = "";
...
//        try {
//            listeIds = (IS_JDBC ? listeIds : dao.loadEntity(queryIds, param, valParam));
//        } catch (Exception ex) {
...
//        }
    ...
    public List<T> executeDynamicQuery(String field, String fieldCount, String entity, String paramsOrder, boolean avance, boolean init, int imax, DaoInterfaceLocal dao) {
        return executeDynamicQuery(field, fieldCount, entity, paramsOrder, avance, init, imax, "id", dao);
    }

    public List<T> executeDynamicQuery(String field, String fieldCount, String entity, String paramsOrder, boolean avance, boolean init, DaoInterfaceLocal dao) {
        return executeDynamicQuery(field, fieldCount, entity, paramsOrder, avance, init, rows, "id", dao);
    }

    public List<T> executeDynamicQuery(String field, String fieldCount, String entity, String paramsOrder, boolean avance, boolean init, int imax, String fieldId, DaoInterfaceLocal dao) {
        return executeDynamicQuery(true, field, fieldCount, entity, paramsOrder, avance, init, imax, fieldId, dao);
    }

    public List<T> executeDynamicQuery(boolean count, String field, String fieldCount, String entity, String paramsOrder, boolean avance, boolean init, int imax, String fieldId, DaoInterfaceLocal dao) {
        String alias = field;
        IS_JDBC = false;
        this.dao = dao;
        if (count) {
...
//            queryIds = queryIds + " ORDER BY " + paramsOrder;
    ...
    public List<ParametreRequete> ajouteParam(ParametreRequete p, List<ParametreRequete> lp) {
        if (p.getAttribut() != null && !(p.getOperation().trim().equals("IS NOT NULL") || p.getOperation().trim().equals("IS NULL")) && !p.isCompareAttribut()) {
        ...
        if (!p.getOtherExpression().isEmpty()) {
            for (ParametreRequete pp : p.getOtherExpression()) {
                lp.addAll(ajouteParam(pp, new ArrayList()));
            }
        }
        return lp;
    }
        ...
        if (p.getOperation().trim().equals("BETWEEN") || p.getOperation().trim().equals("NOT BETWEEN")) {
            ...
            if (IS_JDBC) {
                ...
                if (!p.isCompareAttribut()) {
                    ...
                    re += "" + p.getAttribut() + " " + p.getOperation() + "" + ((String) p.getObjet());
                }
            }
        }
        return re;
