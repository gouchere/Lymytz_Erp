...
BEGIN
	...
	LOOP
		...
		FOR ligne_  IN SELECT DISTINCT COALESCE(c.depot_livraison_prevu, 0) AS depot_livraison_prevu FROM yvs_com_contenu_doc_vente c 
				...
				WHERE e.agence = agence_ AND e.date_entete BETWEEN dates_.date_debut AND dates_.date_fin AND d.type_doc='FV' AND c.conditionnement = cond_
		LOOP
			...
			UPDATE yvs_com_contenu_doc_vente y SET pr = pr_
												FROM yvs_com_doc_ventes d INNER JOIN yvs_com_entete_doc_vente e ON d.entete_doc = e.id 
												WHERE d.id = y.doc_vente AND y.conditionnement = cond_ AND e.date_entete BETWEEN dates_.date_debut AND dates_.date_fin AND COALESCE(y.depot_livraison_prevu, 0) = ligne_.depot_livraison_prevu;
		END LOOP;
