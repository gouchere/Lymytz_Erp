			...
			if(NEW.valider = true)then
				...
				IF(taux_validation_>=100 AND COALESCE(NEW.total_presence,0)>0) THEN
					...
					IF(last_.heure_sortie IS NOT null) THEN
						retardF_ = ((select extract(hour from last_.heure_sortie)) + ((select extract(minute from last_.heure_sortie))/60))-
									((select extract(hour from NEW.heure_fin)) + ((select extract(minute from NEW.heure_fin))/60));
