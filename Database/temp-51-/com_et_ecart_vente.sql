...
declare 
   ...
   versement_reel_s double precision default 0;
   ...
   date_fin_s DATE DEFAULT '01-01-2000';
   date_debut_initial_ DATE DEFAULT date_debut_ - interval '1 day';
   
begin 	
	DROP TABLE IF EXISTS table_et_ecart_vente;
	CREATE TEMP TABLE IF NOT EXISTS table_et_ecart_vente(_agence bigint, _element bigint, _code character varying, _nom character varying, _entete character varying, _versement_attendu double precision, _versement_reel double precision, _ecart double precision, _manquant double precision, _excedent double precision, _solde double precision, _rang integer, _solde_initial boolean, _manquant_planifier double precision, _manquant_traite double precision, _manquant_hors_limit boolean);
	...
	LOOP
		...
		versement_reel_ = 0;
		IF(by_point_)THEN
			FOR users_ IN SELECT h.users as id, e.date_entete, (e.date_entete + interval '1 day')::date as date_fin, t.heure_debut, t.heure_fin
				FROM yvs_com_entete_doc_vente e INNER JOIN yvs_com_creneau_horaire_users h ON e.creneau = h.id INNER JOIN yvs_com_creneau_point c ON h.creneau_point = c.id INNER JOIN yvs_grh_tranche_horaire t On c.tranche = t.id
				WHERE c.point = 53 AND e.date_entete BETWEEN date_initial_ AND date_debut_initial_
			LOOP
				date_fin_s = users_.date_entete;
				IF(users_.heure_debut > users_.heure_fin)THEN
					date_fin_s = users_.date_fin;
				END IF;
				query_montant_ = 'SELECT SUM(y.montant) FROM yvs_compta_caisse_piece_virement y WHERE y.caissier_source = '||users_.id;
				query_montant_ = query_montant_ || ' AND y.statut_piece = ''P'' AND y.date_paiement::date BETWEEN '||QUOTE_LITERAL(users_.date_entete)||' AND '||QUOTE_LITERAL(date_fin_s);
				EXECUTE query_montant_ INTO versement_reel_s;
				versement_reel_ = COALESCE(versement_reel_, 0) + COALESCE(versement_reel_s, 0);
			END LOOP;
		ELSE
		...
		solde_ = ecart_;
		IF(ecart_ != 0)THEN
			INSERT INTO table_et_ecart_vente VALUES(vendeur_.agence, vendeur_.id::bigint, vendeur_.code, vendeur_.nom, 'ECART INITIAL', versement_attendu_, versement_reel_, ecart_, manquant_, excedent_, solde_, i, true, 0, 0, manquant_hors_limit_);
		END IF;
		...
		LOOP
			...
			versement_reel_ = 0;
			IF(by_point_)THEN
				FOR users_ IN SELECT h.users as id, e.date_entete, (e.date_entete + interval '1 day')::date as date_fin 
					FROM yvs_com_entete_doc_vente e INNER JOIN yvs_com_creneau_horaire_users h ON e.creneau = h.id , t.heure_debut, t.heure_fin INNER JOIN yvs_com_creneau_point c ON h.creneau_point = c.id INNER JOIN yvs_grh_tranche_horaire t On c.tranche = t.id  
					WHERE c.point = 53 AND e.date_entete BETWEEN dates_.date_debut AND dates_.date_fin
				LOOP
					date_fin_s = users_.date_entete;
					IF(users_.heure_debut > users_.heure_fin)THEN
						date_fin_s = users_.date_fin;
			...
			IF(versement_attendu_ != 0 or versement_reel_ != 0)THEN
				...
				INSERT INTO table_et_ecart_vente VALUES(vendeur_.agence, vendeur_.id::bigint, vendeur_.code, vendeur_.nom, jour_, versement_attendu_, versement_reel_, ecart_, manquant_, excedent_, solde_, i, false, COALESCE(manquant_planifier_, 0), COALESCE(manquant_traite_, 0), manquant_hors_limit_);
			END IF;
		END LOOP;
		i = i + 1;
		...
		IF(COALESCE(manquant_traite_, 0) > 0)THEN
			INSERT INTO table_et_ecart_vente VALUES(vendeur_.agence, vendeur_.id::bigint, vendeur_.code, vendeur_.nom, 'MANQUANTS', 0, 0,0, 0, 0, 0, i, false, -1, COALESCE(manquant_traite_, 0), manquant_hors_limit_);
		END IF;
	END LOOP;
