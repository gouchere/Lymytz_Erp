-- Function: com_et_ca_achat_by_periode(bigint, bigint, character varying, date, date, character varying, character varying)
-- DROP FUNCTION com_et_ca_achat_by_periode(bigint, bigint, character varying, date, date, character varying, character varying);
CREATE OR REPLACE FUNCTION com_et_ca_achat_by_periode(IN societe_ bigint, IN agence_ bigint, IN element_ character varying, IN date_debut_ date, IN date_fin_ date, IN periode_ character varying, IN nature_ character varying)
...
begin 	
	...
	ELSE
		query_ = 'SELECT y.id::bigint, UPPER(y.code_fsseur) as code, UPPER(CONCAT(y.nom, '' '', y.prenom)) as intitule FROM yvs_base_fournisseur y INNER JOIN yvs_base_tiers a ON y.tiers = a.id WHERE y.code_fsseur IS NOT NULL';
	END IF;
	...
	LOOP
		...
		LOOP
			query_valeur_ = 'SELECT SUM(y.prix_total) FROM yvs_com_contenu_doc_achat y INNER JOIN yvs_com_doc_achats d ON d.id = y.doc_achat 
				INNER JOIN yvs_base_articles a ON y.article = a.id LEFT JOIN yvs_base_depots e ON d.depot_reception = e.id
				WHERE d.type_doc = ''FA'' AND d.statut = ''V'' AND d.document_lie IS NULL AND d.date_doc BETWEEN '''||dates_.date_debut||''' AND '''||dates_.date_fin||'''';
			IF(COALESCE(nature_, 'C') = 'C')THEN
			...
			ELSE
				query_valeur_ = query_valeur_ || ' AND d.fournisseur = '||ligne_.id;
			END IF;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION com_et_ca_achat_by_periode(bigint, bigint, character varying, date, date, character varying, character varying)
  OWNER TO postgres;
