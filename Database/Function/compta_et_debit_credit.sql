-- Function: compta_et_debit_credit(bigint, bigint, bigint, date, date, bigint, character varying, boolean, character varying, boolean)
-- DROP FUNCTION compta_et_debit_credit(bigint, bigint, bigint, date, date, bigint, character varying, boolean, character varying, boolean);
CREATE OR REPLACE FUNCTION compta_et_debit_credit(IN agence_ bigint, IN societe_ bigint, IN valeur_ bigint, IN date_debut_ date, IN date_fin_ date, IN journal_ bigint, IN type_ character varying, IN general boolean, IN table_tiers_ character varying, IN delete_ boolean)
...
begin 	
	CREATE TEMP TABLE IF NOT EXISTS table_debit_credit(_debit_ double precision, _credit_ double precision);
	IF(delete_)THEN
		DELETE FROM table_debit_credit;
	END IF;
	IF(type_ = 'A')THEN
	...
	IF(type_ = 'T')THEN
		...
		INSERT INTO table_debit_credit VALUES(result_.debit, result_.credit);
	ELSIF(type_ = 'C')THEN
		query_ = query_ ||' and c.compte_general = '||valeur_;
		EXECUTE query_ INTO result_;
		SELECT INTO count_ COUNT(*) FROM table_debit_credit;
		IF(COALESCE(count_, 0) > 0)THEN
			UPDATE table_debit_credit SET _debit_ = COALESCE(_debit_, 0) +  COALESCE(result_.debit, 0), _credit_ = COALESCE(_credit_, 0) +  COALESCE(result_.credit, 0);
		ELSIF(COALESCE(result_.debit, 0) > 0 OR COALESCE(result_.credit, 0) > 0)THEN
			INSERT INTO table_debit_credit VALUES(COALESCE(result_.debit, 0), COALESCE(result_.credit, 0));
		END IF;
		IF(general)THEN
			...
			IF(compte_.id IS NOT NULL AND compte_.id > 0)then
				IF(compte_.type_compte = 'CO')THEN
					...
					LOOP
						PERFORM compta_et_debit_credit(agence_ , societe_ , comptes_.id, date_debut_, date_fin_, journal_, type_, general, table_tiers_, false);
					END LOOP;
				END IF;
			END IF;
		END IF;
	ELSE
		...
		INSERT INTO table_debit_credit VALUES(result_.debit, result_.credit);
	END IF;
	return QUERY SELECT * FROM table_debit_credit;
end;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100
  ROWS 1000;
ALTER FUNCTION compta_et_debit_credit(bigint, bigint, bigint, date, date, bigint, character varying, boolean, character varying, boolean)
  OWNER TO postgres;
