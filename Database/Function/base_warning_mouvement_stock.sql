	...
	IF(run_) THEN 		
			...
			IF(line_.id IS NOT NULL) THEN
				IF(COALESCE(line_.stock_min, 0) > 0 OR COALESCE(line_.stock_alert, 0) > 0) THEN
					...
					IF(stock_ < line_.stock_alert) THEN
						...
						IF(COALESCE(mouv_, 0) < 1) THEN
							INSERT INTO yvs_workflow_alertes(model_doc, nature_alerte, id_element, silence, date_save, date_update, date_doc, author, agence, niveau)
									VALUES (model_, 'STOCK ALERTE', line_.id, FALSE, current_timestamp, current_timestamp, date_, author_, agence_, (duree_/delai_)::integer);
						ELSE
					...
					IF(stock_ < line_.stock_min) THEN
						...
						IF(COALESCE(mouv_, 0) < 1) THEN
							INSERT INTO yvs_workflow_alertes(model_doc, nature_alerte, id_element, silence, date_save, date_update, date_doc, author, agence, niveau)
									VALUES (model_, 'STOCK SECURITE', line_.id, FALSE, current_timestamp, current_timestamp, date_, author_, agence_, (duree_/ delai_)::integer);
						ELSE
							UPDATE yvs_workflow_alertes SET date_update = current_timestamp, date_doc = date_, silence = FALSE WHERE id = mouv_;
						END IF;
					END IF;
		...
		IF(COALESCE(model_, 0) > 0)THEN
			...
			IF(COALESCE(mouv_, 0) > 0) THEN
				DELETE FROM yvs_workflow_alertes WHERE id = mouv_;
			END IF;
		END IF;
		...
		IF(COALESCE(model_, 0) > 0)THEN
			IF(action_ = 'INSERT' OR action_ = 'UPDATE') THEN  
				...
				IF(COALESCE(val_ecart,0) > 0 AND (ABS(NEW.cout_stock - last_pr_) > ABS(val_ecart)))THEN
					SELECT INTO mouv_ id FROM yvs_workflow_alertes WHERE id_element = NEW.id AND model_doc = model_;
					IF(COALESCE(mouv_, 0) > 0)THEN
						UPDATE yvs_workflow_alertes SET silence = FALSE WHERE id = mouv_;
					ELSE
						INSERT INTO yvs_workflow_alertes(model_doc, nature_alerte, id_element, silence, date_save, date_update, date_doc, author, agence, niveau)
								VALUES (model_, 'CONFIRMATION', NEW.id, FALSE, current_timestamp, current_timestamp, date_, author_, agence_, 1);
					END IF;
				END IF;
			ELSE
