	...
	IF((valeur_ IS NOT NULL AND valeur_ > 0) AND (compte_ IS NOT NULL AND compte_ > 0))THEN
		IF(table_ = 'DOC_ACHAT')THEN
			query_ = 'SELECT DISTINCT ap.centre AS centre, (('||valeur_||') *(COALESCE(ap.coefficient, 1)/100)) AS montant, ap.coefficient AS coeficient FROM yvs_compta_centre_contenu_achat ap 
				...
				WHERE (COALESCE(ap.coefficient,0)!=0) AND ca.compte = '||compte_||' AND co.doc_achat = '||element_;
		ELSIF(table_ = 'FRAIS_MISSION')THEN
			query_ = 'SELECT DISTINCT ap.centre AS centre, (('||valeur_||') *(COALESCE(ap.coefficient, 1)/100)) AS montant, ap.coefficient AS coeficient FROM yvs_compta_centre_mission ap 
				...
				WHERE (COALESCE(ap.coefficient,0)!=0) AND om.compte_charge = '||compte_||' AND mi.id = '||element_;
		ELSE
