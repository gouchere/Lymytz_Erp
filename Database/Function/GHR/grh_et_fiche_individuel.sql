...
-- 	DROP TABLE table_fiche_individuel;
	...
	FOR element_ in SELECT e.element_salaire as id, e.libelle, coalesce(y.num_sequence, 0) as numero, coalesce(e.ordre, 0) as ordre, y.retenue, e.groupe_element as groupe 
						FROM yvs_stat_grh_element_dipe e 
						INNER JOIN yvs_stat_grh_etat s on s.id = e.etat 
	...
	loop
		...
		loop
			...
			if(element_.retenue is false)then
				SELECT INTO valeur_ coalesce(d.montant_payer, 0) FROM yvs_grh_detail_bulletin d 
																 INNER JOIN yvs_grh_bulletins b on d.bulletin = b.id 
																 INNER JOIN yvs_grh_contrat_emps c on b.contrat = c.id 
																 INNER JOIN yvs_grh_ordre_calcul_salaire o on b.entete = o.id
																 WHERE d.element_salaire = element_.id and c.employe = employe_ 
																 and o.debut_mois >= dates_.date_debut and o.fin_mois <= dates_.date_fin;
			else
