-- Function: com_recalcule_pr_periode_vente(date, date)
-- DROP FUNCTION com_recalcule_pr_periode_vente(date, date);
CREATE OR REPLACE FUNCTION com_recalcule_pr_periode_vente(agence_ bigint, debut_ date, fin_ date)
  RETURNS boolean AS
$BODY$   
DECLARE	
	ligne_ RECORD;
...
BEGIN
	...
	WHERE c.doc_vente=d.id and e.agence=agence_ AND e.date_entete BETWEEN debut_ AND fin_ and d.type_doc='FV';
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION com_recalcule_pr_periode_vente(bigint, date, date)
  OWNER TO postgres;
