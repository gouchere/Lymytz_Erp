...
CREATE OR REPLACE FUNCTION prod_et_production_vente(IN societe_ bigint, IN agence_ bigint, IN articles_ character varying, IN date_debut_ date, IN date_fin_ date, IN periode_ character varying, IN type_ character varying)
...
BEGIN 	
	RETURN QUERY SELECT * FROM prod_et_production_vente(societe_, agence_, articles_, date_debut_, date_fin_, periode_, type_, 'R');
END;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100
  ROWS 1000;
ALTER FUNCTION prod_et_production_vente(bigint, bigint, character varying, date, date, character varying, character varying)
  OWNER TO postgres;
