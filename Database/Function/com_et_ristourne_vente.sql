-- Function: com_et_ristourne_vente(bigint, bigint, bigint, bigint, bigint, date, date, character varying, integer)
DROP FUNCTION com_et_ristourne_vente(bigint, bigint, bigint, bigint, bigint, date, date, character varying, integer);
CREATE OR REPLACE FUNCTION com_et_ristourne_vente(IN societe_ bigint, IN agence_ bigint, IN zone_ bigint, IN users_ bigint, IN client_ bigint, IN date_debut_ date, IN date_fin_ date, IN period_ character varying, IN cumul_ integer)
...
declare 
   ...
   ristourne_ CHARACTER VARYING DEFAULT 'SELECT u.id, u.code_users, u.nom_users, c.ristourne, c.quantite, c.prix_total, c.conditionnement, d.id as facture, d.num_doc';
...
begin 	
	...
	-- Si cumul=2 On cumule par client (récapitulatif)
	-- Si cumul=3 On cumule par Article Vendeur
	-- Si cumul=4 On ne cumule pas (par facture)
	DROP TABLE IF EXISTS table_et_ristourne_vente;
	CREATE TEMP TABLE IF NOT EXISTS table_et_ristourne_vente(_client bigint, _code character varying, _nom character varying, _users bigint, _code_users character varying, _nom_users character varying, _unite bigint, _entete character varying, _quantite double precision, _prix_total double precision, _ristourne double precision, _rang integer, _id_facture bigint, _numero_facture character varying);
	...
	END IF;
	RAISE NOTICE 'where_ %',where_;
	IF(cumul_=0)THEN
		
	ELSIF(cumul_=1) THEN --Par client et par article
		ristourne_ = 'SELECT y.id, c.article, SUM(COALESCE(c.ristourne, 0)) as ristourne, SUM(COALESCE(c.quantite, 0)) as quantite, SUM(COALESCE(c.prix_total, 0)) as prix_total, c.conditionnement, null::bigint AS facture, null AS num_doc';
	ELSIF(cumul_=2) THEN --Par clients
		ristourne_ = 'SELECT y.id, y.code_client, SUM(COALESCE(c.ristourne, 0)) as ristourne, SUM(COALESCE(c.quantite, 0)) as quantite, SUM(COALESCE(c.prix_total, 0)) as prix_total, null::bigint AS facture, null AS num_doc';
	ELSIF(cumul_=3) THEN --Par article et par vendeur
		ristourne_ = 'SELECT u.id, u.code_users, u.nom_users, SUM(COALESCE(c.ristourne, 0)) as ristourne, SUM(COALESCE(c.quantite, 0)) as quantite, SUM(COALESCE(c.prix_total, 0)) as prix_total, c.conditionnement, null::bigint AS facture, null AS num_doc';
	END IF;
	...
	LOOP
		...
		LOOP
			...
			query_ = ristourne_ || save_ || where_ || ' AND e.date_entete BETWEEN'||QUOTE_LITERAL(dates_.date_debut)||' AND '||QUOTE_LITERAL(dates_.date_fin)||' AND y.id = '||clients_.id;
			...
			FOR data_ IN EXECUTE query_
				LOOP
					...
					IF(COALESCE(data_.ristourne, 0) != 0)THEN
						INSERT INTO table_et_ristourne_vente VALUES(clients_.id, clients_.code_client, clients_.nom, data_.id, code_user_, name_user_, cond_, jour_, data_.quantite, data_.prix_total, data_.ristourne, i, data_.facture, data_.num_doc);
					END IF;
			END LOOP;
		END LOOP;
	END LOOP;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION com_et_ristourne_vente(bigint, bigint, bigint, bigint, bigint, date, date, character varying, integer)
  OWNER TO postgres;
