...
BEGIN 	
	...
	IF(COALESCE(model_.id, 0) > 0)THEN
		...
		SELECT INTO delai_ COALESCE(w.ecart, 0) FROM yvs_warning_model_doc w WHERE w.model = model_.id AND w.societe=(select a.societe FROM yvs_agences a WHERE a.id=agence_ limit 1);
