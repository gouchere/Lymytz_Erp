...
BEGIN	
	IF(EXEC_) THEN
		...
		IF(action_='INSERT' OR action_='UPDATE') THEN
			SELECT INTO line_  cr.agence, pv.numero as num_doc, pv.users, cr.nom_users as nom FROM yvs_com_ecart_entete_vente pv INNER JOIN yvs_users cr ON cr.id=pv.users WHERE pv.id=NEW.piece; 									    
