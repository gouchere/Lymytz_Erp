-- Function: grh_et_journal_paie_by_convention(bigint, character varying, character varying, bigint)
-- DROP FUNCTION grh_et_journal_paie_by_convention(bigint, character varying, character varying, bigint);
CREATE OR REPLACE FUNCTION grh_et_journal_paie_by_convention(IN societe_ bigint, IN header_ character varying, IN agence_ character varying, IN categorie_ bigint)
...
-- 	DROP TABLE sub_table_journal_paie_by_convention;
	...
	IF((header_ IS NOT null and header_ != '') and (type_ IS NOT null))then
		...
		LOOP
			...
			LOOP
				IF(COALESCE(regles_.by_formulaire, false))THEN
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION grh_et_journal_paie_by_convention(bigint, character varying, character varying, bigint)
  OWNER TO postgres;
