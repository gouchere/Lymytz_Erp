...
begin 	
	...
	ELSIF(table_externe_ = 'yvs_com_contenu_doc_stock_reception') THEN
			SELECT INTO name_  d.num_doc FROM yvs_com_contenu_doc_stock_reception r INNER JOIN yvs_com_contenu_doc_stock c ON c.id=r.contenu 
			INNER JOIN yvs_com_doc_stocks d ON d.id=c.doc_stock WHERE r.id=id_externe_;
	...
	ELSIF(table_externe_ = 'yvs_com_contenu_doc_vente') THEN
			SELECT INTO name_ d.num_doc FROM yvs_com_contenu_doc_vente c INNER JOIN yvs_com_doc_ventes d ON d.id=c.doc_vente INNER JOIN yvs_com_entete_doc_vente e ON e.id=d.entete_doc 
			INNER JOIN yvs_com_creneau_horaire_users cu ON cu.id=e.creneau 
			INNER JOIN yvs_users u ON u.id=cu.users WHERE c.id=id_externe_ limit 1;
	ELSE 
		name_='';
	END IF;
	RETURN name_;
