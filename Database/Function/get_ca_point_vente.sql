-- Function: get_ca_point_vente(bigint, date, date)
-- DROP FUNCTION get_ca_point_vente(bigint, date, date);
	...
	IF(id_ IS NOT NULL AND id_>0) THEN
		IF(date_debut_ IS NULL OR date_fin_ IS NULL) THEN					
			...
			SELECT INTO total_ SUM(c.prix_total-c.ristourne) FROM yvs_com_contenu_doc_vente c INNER JOIN yvs_com_doc_ventes d ON c.doc_vente = d.id 
				...
				WHERE d.type_doc = 'FV' AND d.statut = 'V' AND p.point=id_;
		...
		ELSE
			...
			SELECT INTO total_ SUM(c.prix_total-c.ristourne) FROM yvs_com_contenu_doc_vente c INNER JOIN yvs_com_doc_ventes d ON c.doc_vente = d.id 
				INNER JOIN yvs_com_entete_doc_vente e ON d.entete_doc = e.id INNER JOIN yvs_com_creneau_horaire_users h ON e.creneau = h.id INNER JOIN yvs_com_creneau_point p ON h.creneau_point = p.id
				WHERE d.type_doc = 'FV' AND d.statut = 'V' AND p.point=id_ AND e.date_entete BETWEEN date_debut_ AND date_fin_;
			SELECT INTO avoir_ SUM(c.prix_total) FROM yvs_com_contenu_doc_vente c INNER JOIN yvs_com_doc_ventes da ON c.doc_vente = da.id 
				INNER JOIN yvs_com_entete_doc_vente e ON da.entete_doc = e.id INNER JOIN yvs_com_creneau_horaire_users h ON e.creneau = h.id
				INNER JOIN yvs_com_creneau_point p ON h.creneau_point = p.id
				WHERE da.type_doc = 'FAV' AND da.statut = 'V' AND p.point=id_ AND e.date_entete BETWEEN date_debut_ AND date_fin_;
			SELECT INTO cs_p SUM(o.montant) FROM yvs_com_cout_sup_doc_vente o INNER JOIN yvs_grh_type_cout t ON o.type_cout = t.id 
				INNER JOIN yvs_com_doc_ventes d ON o.doc_vente = d.id INNER JOIN yvs_com_entete_doc_vente e ON d.entete_doc = e.id
				INNER JOIN yvs_com_creneau_horaire_users h ON e.creneau = h.id INNER JOIN yvs_com_creneau_point p ON h.creneau_point = p.id
				WHERE d.type_doc = 'FV' AND d.statut = 'V' AND p.point=id_ AND e.date_entete BETWEEN date_debut_ AND date_fin_;
