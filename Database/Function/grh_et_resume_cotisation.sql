...
$BODY$
declare 
	titre_ character varying default 'RES_COT';
...
begin 	
	...
	if((header_ is not null and header_ !='') and (agence_ is not null and agence_ !=''))then
		for element_ in select e.element_salaire as id, coalesce(e.groupe_element, 0) as groupe, e.libelle 
		from yvs_stat_grh_element_dipe e inner join yvs_stat_grh_etat s on s.id = e.etat where s.code = titre_
		loop
			...
			and h.agence::character varying in (select val from regexp_split_to_table(agence_,',') val) and (b.statut IN ('V', 'P'));
			...
			where b.entete::character varying in (select val from regexp_split_to_table(header_,',') val) and e.id = element_.id 
			and h.agence::character varying in (select val from regexp_split_to_table(agence_,',') val) and y.civilite = 'M.' and (b.statut IN ('V', 'P'));
