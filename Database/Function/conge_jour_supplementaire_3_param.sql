-- Function: conge_jour_supplementaire(bigint, date, bigint)

-- DROP FUNCTION conge_jour_supplementaire(bigint, date, bigint);

CREATE OR REPLACE FUNCTION conge_jour_supplementaire(id_ bigint, date_jour_ date, exercice_ bigint)
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION conge_jour_supplementaire(bigint, date, bigint)
  OWNER TO postgres;
COMMENT ON FUNCTION conge_jour_supplementaire(bigint, date, bigint) IS 'fonction qui retourne le total des jours de conge supplémentaire d''un employé par un employe au long de son exercice en fonction de sa derniere date de cloture des conges
