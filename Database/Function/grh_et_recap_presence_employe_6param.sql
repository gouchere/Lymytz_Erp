...
begin 	
	...
	ELSE
		_query = 'SELECT e.id, e.matricule, e.civilite, e.nom, e.prenom, e.date_embauche, e.agence, c.conge_acquis, c.source_first_conge, c.date_first_conge 
			FROM yvs_grh_employes e INNER JOIN yvs_grh_contrat_emps c ON c.employe = e.id INNER JOIN yvs_agence a ON a.id = e.agence WHERE e.actif IS TRUE AND a.societe = '||societe_||' ORDER BY e.nom';
	END IF;
