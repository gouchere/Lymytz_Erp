-- DROP FUNCTION public.valorisation_stock(int8, int8, int8, int8, float8, float8, varchar, int8, varchar, date);

CREATE OR REPLACE FUNCTION public.valorisation_stock(article_ bigint, conditionnement_ bigint, depot_ bigint, tranche_ bigint, quantite_ double precision, cout_ double precision, tableexterne_ character varying, idexterne_ bigint, mouvement_ character varying, date_ date)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
DECLARE 
...
BEGIN
	select into meth_ COALESCE(methode_val, 'CMPI') from yvs_base_articles where id = article_;
