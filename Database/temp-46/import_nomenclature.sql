...
BEGIN
	...
	LOOP
		query_ = 'SELECT n.*, u.reference AS unite FROM yvs_prod_nomenclature n INNER JOIN yvs_base_articles a ON n.article = a.id LEFT JOIN yvs_base_conditionnement c ON n.unite_mesure = c.id LEFT JOIN yvs_base_unite_mesure u ON c.unite = u.id INNER JOIN yvs_base_famille_article f ON a.famille = f.id WHERE f.societe = '||societe_||' AND a.ref_art = '||QUOTE_LITERAL(articles_.ref_art);
		...
		LOOP
			...
			ELSE
				UPDATE yvs_prod_nomenclature SET reference = nomenclatures_.reference, niveau = nomenclatures_.niveau, debut_validite = nomenclatures_.debut_validite, fin_validite = nomenclatures_.fin_validite, quantite = nomenclatures_.quantite, actif = nomenclatures_.actif, quantite_lie_aux_composants = nomenclatures_.quantite_lie_aux_composants, 
					principal = nomenclatures_.principal, alway_valide = nomenclatures_.alway_valide, unite_preference = nomenclatures_.unite_preference, date_update = nomenclatures_.date_update, date_save = nomenclatures_.date_save, for_conditionnement = nomenclatures_.for_conditionnement, marge_qte = nomenclatures_.marge_qte, acces = nomenclatures_.acces, masquer = nomenclatures_.masquer, type_nomenclature = nomenclatures_.type_nomenclature
				WHERE id = nomenclature_;
			...
			LOOP
				SELECT INTO article_ y.id FROM yvs_base_articles y INNER JOIN yvs_base_famille_article f ON y.famille = f.id WHERE f.societe = societe_ AND y.ref_art = composants_.ref_art;
				IF(COALESCE(article_, 0) > 1)THEN
					...
					IF(COALESCE(composant_, 0) < 1)THEN
						...
						composant_ = currval('yvs_prod_composant_nomenclature_id_seq');
					ELSE
						UPDATE yvs_prod_composant_nomenclature SET quantite = composants_.quantite, coefficient = composants_.coefficient, type = composants_.type, mode_arrondi = composants_.mode_arrondi, actif = composants_.actif, 
										date_update = composants_.date_update, date_save = composants_.date_save, stockable = composants_.stockable, ordre = composants_.ordre, inside_cout = composants_.inside_cout, alternatif = composants_.alternatif, free_use = composants_.free_use
						WHERE id = composant_;
					END IF;
				END IF;
			END LOOP;
