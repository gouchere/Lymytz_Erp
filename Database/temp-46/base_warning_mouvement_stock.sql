		...
		IF(COALESCE(model_, 0) > 0)THEN
			IF(action_ = 'INSERT' OR action_ = 'UPDATE') THEN  
				...
				IF(COALESCE(taux_ecart, 0) > 0)THEN
					...
					SELECT INTO nb_line COUNT (id) FROM yvs_base_mouvement_stock WHERE conditionnement=NEW.conditionnement AND depot=NEW.depot AND id!=NEW.id;
					IF(COALESCE(nb_line,0)>0) THEN
