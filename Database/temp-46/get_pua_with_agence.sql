CREATE OR REPLACE FUNCTION get_pua(article_ bigint, fsseur_ bigint, depot_ bigint, unite_ bigint, date_ date, agence_ bigint)
...
DECLARE
	...
	query_ character varying default 'select c.pua_recu, c.conditionnement from yvs_com_contenu_doc_achat c inner join yvs_com_doc_achats d on d.id = c.doc_achat
	where d.type_doc = ''FA'' AND d.statut = ''V'' AND c.article = '||article_;
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION get_pua(bigint, bigint, bigint, bigint, date, bigint)
  OWNER TO postgres;