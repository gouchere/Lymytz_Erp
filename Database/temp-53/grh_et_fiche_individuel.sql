-- FUNCTION: public.grh_et_fiche_individuel(bigint, date, date, character varying)

DROP FUNCTION IF EXISTS public.grh_et_fiche_individuel(bigint, date, date, character varying);

CREATE OR REPLACE FUNCTION public.grh_et_fiche_individuel(
	employe_ bigint,
	debut_ date,
	fin_ date,
	periode_ character varying,
	brouillon boolean)
RETURNS TABLE(element bigint, numero integer, libelle character varying, groupe bigint, entete character varying, montant double precision, rang integer, is_group boolean, is_total boolean) 
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE 
    ROWS 1000
...
ALTER FUNCTION public.grh_et_fiche_individuel(bigint, date, date, character varying, boolean)
    OWNER TO postgres;
