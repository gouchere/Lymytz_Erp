-- FUNCTION: public.grh_et_recap_dipe_fne(bigint, character varying, character varying)

DROP FUNCTION IF EXISTS public.grh_et_recap_dipe_fne(bigint, character varying, character varying);

CREATE OR REPLACE FUNCTION public.grh_et_recap_dipe_fne(
	societe_ bigint,
	agence_ character varying,
	header_ character varying,
	brouillon boolean)
RETURNS TABLE(employe bigint, matricule character varying, nom character varying, entete character varying, valeur double precision, montant double precision, is_taux boolean) 
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE 
    ROWS 1000
...
ALTER FUNCTION public.grh_et_recap_dipe_fne(bigint, character varying, character varying, boolean)
    OWNER TO postgres;
