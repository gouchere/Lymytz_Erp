		...
		if(formulaire_ is true)then
			...
			OR upper(regle_.libelle) = upper('Net a payé'))THEN				
				if(type_ = 'E')then
                    ...
                    and ((brouillon_ IS FALSE AND yvs_grh_bulletins.statut IN ('V', 'P')) OR (brouillon_ IS TRUE AND yvs_grh_bulletins.statut IS NOT NULL));
				elsif(type_ = 'S')then
					...
					SELECT INTO valeur_ SUM(d.montant_payer - d.retenu_salariale) AS Salaire 
						...
						and h.id_service = service_ and ((brouillon_ IS FALSE AND b.statut IN ('V', 'P')) OR (brouillon_ IS TRUE AND b.statut IS NOT NULL));	
					if(with_sous_ is true)then
						...
						loop
							select into sous_valeur_ SUM(d.montant_payer - d.retenu_salariale) AS Salaire 
								...
								and (d.montant_payer > 0 or d.retenu_salariale > 0 or d.montant_employeur > 0)
								and h.agence::character varying in (select val from regexp_split_to_table(agence_,',') val) 
								and h.id_service = sous_ and ((brouillon_ IS FALSE AND b.statut IN ('V', 'P')) OR (brouillon_ IS TRUE AND b.statut IS NOT NULL));	
...
ALTER FUNCTION public.grh_get_valeur_formulaire_dipe(character varying, bigint, character varying, bigint, character varying, boolean)
    OWNER TO postgres;
