...
declare 
   ...
   query_ CHARACTER VARYING DEFAULT 'SELECT u.id, u.code_users, u.nom_users FROM yvs_users u INNER JOIN yvs_agences a ON u.agence = a.id
...
begin 	
	...
	loop
		FOR data_ IN SELECT a.id, a.ref_art, a.designation, c.id as conditionnement, c.prix, c.prix_achat, u.reference, 
			...
			order by a.id, d.type_doc
		loop
			excedent_ = 0;
			quantite_ = 0;
			update_ = false;
			IF(data_.type_doc = 'ES')then 
			...
			if(quantite_ > 0)then	
				if(update_ = false)then
					...
					insert into table_et_valorise_inventaire values (users_.id, users_.code_users, users_.nom_users, 
						...
						data_.conditionnement, data_.reference, quantite_, prix_, total_);
				else
					total_ = quantite_ * coalesce(prec_._prix_, 0) * COALESCE(coefficient_, 1);
					update table_et_valorise_inventaire set _quantite_ = quantite_, _total_ = total_ where _users_ = users_.id and _unite_ = data_.conditionnement;
				end if;
			end if;
		END LOOP;
	end loop;
