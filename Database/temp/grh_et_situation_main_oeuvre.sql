-- FUNCTION: public.grh_et_situation_main_oeuvre(bigint, character varying, bigint, boolean)

DROP FUNCTION IF EXISTS public.grh_et_situation_main_oeuvre(bigint, character varying, bigint, boolean);

CREATE OR REPLACE FUNCTION public.grh_et_situation_main_oeuvre(
	societe_ bigint,
	agence_ character varying,
	header_ bigint,
	annuel_ boolean,
	brouillon boolean)
RETURNS TABLE(id bigint, matricule character varying, nom character varying, sexe character varying, situation character varying, categorie character varying, echellon character varying, anciennete integer, cnps character varying, date_naissance date, age integer, date_embauche date, poste character varying, service character varying, salaire double precision) 
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE 
    ROWS 1000
...
BEGIN 	
	...
	IF(annuel_)THEN
		query_ = 'SELECT e.id, concat(e.nom,'' '',e.prenom) AS nom, (SELECT (12* EXTRACT(YEAR FROM age(current_date, ct.date_debut))::integer) + (EXTRACT(MONTH FROM age(current_date, ct.date_debut))::integer)) AS anciennete,
			...
			INNER JOIN yvs_grh_echelons h ON cc.echellon = h.id INNER JOIN yvs_grh_employes e ON ce.employe = e.id INNER JOIN yvs_grh_contrat_emps ct ON ct.employe=e.id INNER JOIN yvs_grh_poste_de_travail p ON e.poste_actif = p.id INNER JOIN yvs_grh_departement d ON p.departement = d.id
			WHERE e.actif = TRUE AND ce.actif = TRUE';
	ELSE
		query_ = 'SELECT e.id, concat(e.nom,'' '',e.prenom) AS nom, (SELECT (12* EXTRACT(YEAR FROM age(current_date, ct.date_debut))::integer) + (EXTRACT(MONTH FROM age(current_date, ct.date_debut))::integer)) AS anciennete,
			e.date_naissance , (SELECT EXTRACT(YEAR FROM current_date::date) - EXTRACT(YEAR FROM e.date_naissance)) AS age, e.matricule, e.etat_civil, c.categorie,
			h.echelon, ct.date_debut, p.intitule as poste, d.intitule as service, e.matricule_cnps as cnps, e.civilite,
			grh_get_brut_annuel('||societe_||', e.id, '||header_||', '||brouillon||') as salaire
			FROM yvs_grh_categorie_professionelle c INNER JOIN yvs_grh_convention_collective cc ON c.id = cc.categorie INNER JOIN  yvs_grh_convention_employe ce ON cc.id = ce.convention
			INNER JOIN yvs_grh_echelons h ON cc.echellon = h.id INNER JOIN yvs_grh_employes e ON ce.employe = e.id INNER JOIN yvs_grh_contrat_emps ct ON ct.employe=e.id INNER JOIN yvs_grh_poste_de_travail p ON e.poste_actif = p.id INNER JOIN yvs_grh_departement d ON p.departement = d.id
			WHERE e.actif = TRUE AND ce.actif = TRUE';
	END IF;
	...
	LOOP
		IF(COALESCE(data_.salaire, 0) > 0)THEN
			INSERT INTO table_situation_main_oeuvre VALUES(data_.id, data_.matricule, data_.nom, data_.civilite, data_.etat_civil, data_.categorie, data_.echelon, data_.anciennete, data_.cnps, data_.date_naissance, data_.age, data_.date_debut, data_.poste, data_.service, COALESCE(data_.salaire, 0));
		END IF;
	END LOOP;
...
ALTER FUNCTION public.grh_et_situation_main_oeuvre(bigint, character varying, bigint, boolean, boolean)
    OWNER TO postgres;
