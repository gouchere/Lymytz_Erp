-- FUNCTION: public.grh_et_recap_dipe_cnps(bigint, character varying, character varying, character varying)

DROP FUNCTION IF EXISTS public.grh_et_recap_dipe_cnps(bigint, character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION public.grh_et_recap_dipe_cnps(
	societe_ bigint,
	agence_ character varying,
	header_ character varying,
	sommable_ character varying,
	brouillon boolean)
RETURNS TABLE(employe bigint, matricule character varying, nom character varying, numero character varying, entete character varying, rang integer, montant double precision, is_sum boolean, is_total boolean) 
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE 
    ROWS 1000
	...
	if(header_ is not null and header_ != '')then	
		...
		loop
			...
			loop
				...
				valeur_ = (select grh_get_valeur_element_dipe(element_.ids, header_, employe_.id, brouillon));
...
ALTER FUNCTION public.grh_et_recap_dipe_cnps(bigint, character varying, character varying, character varying, boolean)
    OWNER TO postgres;
