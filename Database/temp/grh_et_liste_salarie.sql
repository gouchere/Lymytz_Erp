-- FUNCTION: public.grh_et_liste_salarie(bigint, character varying, character varying, integer)

DROP FUNCTION IF EXISTS public.grh_et_liste_salarie(bigint, character varying, character varying, integer);

CREATE OR REPLACE FUNCTION public.grh_et_liste_salarie(
	societe_ bigint,
	header_ character varying,
	agence_ character varying,
	colonne_ integer,
	brouillon boolean)
RETURNS TABLE(id bigint, matricule character varying, nom_prenom character varying, element bigint, numero character varying, libelle character varying, montant double precision, rang integer) 
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE 
    ROWS 1000
	...
	IF((header_ IS NOT null and header_ != ''))then
		...
		LOOP
			IF(regles_.by_formulaire)THEN
				...
				LOOP
					valeur_ = (SELECT grh_get_valeur_formulaire_dipe(agence_, regles_.ids, header_, element_.id, 'E', boolean));
...
ALTER FUNCTION public.grh_et_liste_salarie(bigint, character varying, character varying, integer, boolean)
    OWNER TO postgres;
