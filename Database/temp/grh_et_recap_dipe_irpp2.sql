-- FUNCTION: public.grh_et_recap_dipe_irpp2(bigint, character varying, character varying, character varying)

DROP FUNCTION IF EXISTS public.grh_et_recap_dipe_irpp2(bigint, character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION public.grh_et_recap_dipe_irpp2(
	societe_ bigint,
	agence_ character varying,
	header_ character varying,
	sommable character varying,
	brouillon boolean)
RETURNS TABLE(employe bigint, matricule character varying, nom character varying, entete character varying, rang integer, montant double precision) 
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE 
    ROWS 1000
			...
			loop
				...
				if(element_.retenue is false)then
					select into valeur_ coalesce(d.montant_payer, 0) from yvs_grh_detail_bulletin d inner join yvs_grh_bulletins b on d.bulletin = b.id 
						...
						and b.entete::character varying in (select val from regexp_split_to_table(header_,',') val) and ((brouillon IS FALSE AND b.statut IN ('V', 'P')) OR (brouillon IS TRUE AND b.statut IS NOT NULL));
				else
...
ALTER FUNCTION public.grh_et_recap_dipe_irpp2(bigint, character varying, character varying, character varying, boolean)
    OWNER TO postgres;
