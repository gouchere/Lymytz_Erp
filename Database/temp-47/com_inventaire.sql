...
DECLARE 
	...
	with_lot_ BOOLEAN DEFAULT false;
	...
	colonne_ CHARACTER VARYING DEFAULT 'SELECT DISTINCT y.id, y.article, y.depot, a.ref_art, a.designation, f.reference_famille, f.designation as famille, d.agence, a.actif, 
		c.id AS unite, u.id AS mesure, u.reference, COALESCE(c.prix, a.puv) AS puv, COALESCE(c.prix_achat, a.pua) AS pua, e.emplacement AS emplacement, y.requiere_lot';
...
BEGIN 	
	...
	CREATE TEMP TABLE IF NOT EXISTS table_inventaire_preparatoire(_depot bigint, _article bigint, _code character varying, _designation character varying, numero_ character varying, _famille character varying, _unite bigint, _reference character varying, _emplacement bigint, prix_ double precision, _puv double precision, _pua double precision, _pr double precision, _stock double precision, _reservation double precision, _reste_a_livre double precision, _position double precision, _count double precision, _lot bigint); 
	...
	LOOP
		...
		IF(COALESCE(preparatoire_, FALSE) IS FALSE)THEN
			...
			SELECT INTO reste_a_livre_ ((COALESCE((select sum(c.quantite) from yvs_com_contenu_doc_vente c inner join yvs_base_articles a on a.id=c.article inner join yvs_com_doc_ventes d on d.id=c.doc_vente  inner join yvs_com_entete_doc_vente en on en.id=d.entete_doc
					where d.type_doc = 'FV' and d.statut_livre != 'L' and d.statut = 'V' and en.date_entete <= date_ AND c.article = articles_.article AND d.depot_livrer = articles_.depot AND c.conditionnement = articles_.unite limit 1), 0))
		...
		IF(articles_.requiere_lot IS TRUE)THEN
			FOR lots_ IN SELECT y.id, get_stock(m.article, 0, m.depot, 0, 0, date_, m.conditionnement, l.lot) AS stock 
				FROM yvs_base_mouvement_stock m INNER JOIN yvs_base_mouvement_stock_lot l ON m.id = l.mouvement INNER JOIN yvs_com_lot_reception y ON l.lot = y.id 
					WHERE m.depot = articles_.depot AND m.conditionnement = articles_.unite GROUP BY y.id, m.article, m.depot, m.conditionnement, l.lot 
				HAVING get_stock(m.article, 0, m.depot, 0, 0, date_, m.conditionnement, l.lot) > 0 ORDER BY y.date_expiration, y.date_fabrication, y.numero
			LOOP
				stock_ = stock_ - lots_.stock;
				IF(print_all_)THEN
					IF(lots_.stock != 0)THEN
						insert_ = true;
					ELSE
				...
				IF(insert_)THEN
					IF(type_ = 'A')THEN
						prix_ = COALESCE((SELECT get_pua(articles_.article, 0, depot_, articles_.unite, date_)), 0);
					ELSIF(type_ = 'V')THEN
						prix_ = COALESCE((SELECT get_puv(articles_.article, 0, 0, 0, depot_, 0, date_, articles_.unite, false)), 0);
					ELSE
						prix_ = COALESCE((SELECT get_pr(articles_.article, depot_, 0, date_, articles_.unite)), 0);
					END IF;
					i_count_ = i_count_ + 1;
					INSERT INTO table_inventaire_preparatoire VALUES(articles_.depot, articles_.article, articles_.ref_art, articles_.designation, articles_.reference_famille, articles_.famille, articles_.unite, articles_.reference, articles_.emplacement, prix_, articles_.puv, articles_.pua, 0, lots_.stock, COALESCE(reservation_, 0), COALESCE(reste_a_livre_, 0), position_, COALESCE(count_, 0), lots_.id);
				END IF;
				IF(limit_ IS NOT NULL AND limit_ > 0)THEN
			...
			END LOOP;
		END IF;
		IF((with_lot_ IS TRUE AND stock_ > 0) OR with_lot_ IS FALSE)THEN
			IF(print_all_)THEN
				IF(stock_ != 0)THEN
					insert_ = true;
				ELSE
			...
			IF(insert_)THEN
				...
				END IF;
				i_count_ = i_count_ + 1;
				INSERT INTO table_inventaire_preparatoire VALUES(articles_.depot, articles_.article, articles_.ref_art, articles_.designation, articles_.reference_famille, articles_.famille, articles_.unite, articles_.reference, articles_.emplacement, prix_, articles_.puv, articles_.pua, 0, stock_, COALESCE(reservation_, 0), COALESCE(reste_a_livre_, 0), position_, COALESCE(count_, 0), 0);
			END IF;
			IF(limit_ IS NOT NULL AND limit_ > 0)THEN
				if(i_count_ >= limit_)THEN
					RAISE NOTICE 'EXIT';	
		...
		RAISE NOTICE '(%) % : %ms', i_count_,articles_.ref_art, (fin_ - debut_);
	END LOOP;
