...
update yvs_grh_pointage set valider = false where valider = true 
	and (((select concat(date_debut, ' ',heure_debut)::timestamp from yvs_grh_presence where id = presence) > heure_entree) 
		or ((select concat(date_fin, ' ',heure_fin)::timestamp from yvs_grh_presence where id = presence) < heure_sortie))