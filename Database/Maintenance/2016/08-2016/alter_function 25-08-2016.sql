...
BEGIN
	if(NEW.valider != OLD.valider)then	
		if(NEW.valider = true)then
			...
			requis_ = HS_ - HE_;
			RAISE NOTICE 'HS_ : %',HS_||' - HE_ : '||HE_||' - requis_ : '||requis_;
			if(requis_ is null)then
				requis_ = 0;
			end if;
			if(requis_ < 0)then
			...
			RAISE NOTICE 'total_presence_ : %',total_presence_||' - requis_ : '||requis_;
			update yvs_grh_presence set total_presence = total_presence_, total_heure_sup = total_supplementaire_, total_heure_compensation = total_complementaire_ where id = OLD.id;
		end if;
	end if;
	return NEW; 
END;$BODY$
