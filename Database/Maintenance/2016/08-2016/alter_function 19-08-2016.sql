...
BEGIN
	if(NEW.valider = true and NEW.actif = true and NEW.heure_entree is not null and NEW.heure_sortie is not null)then
		...
		if(NEW.compensation_heure = false and NEW.heure_supplementaire = false) then
			...
			update yvs_grh_presence set total_presence = total_ where id = NEW.presence and valide = false;
		else
			if(NEW.heure_supplementaire = true) then
				update yvs_grh_presence set total_heure_sup = total_heure_sup + somme_ where id = NEW.presence and valide = false;
			end if;
			if(NEW.compensation_heure = true) then
				update yvs_grh_presence set total_heure_compensation = total_heure_compensation + somme_ where id = NEW.presence and valide = false;
			end if;	
		end if;		
	end if;
		...
		if((OLD.valider != NEW.valider or OLD.actif != NEW.actif))then -- Si c'est le cas
			...
			if(NEW.valider = true and NEW.actif = true)then -- Si c'est le cas on ajout l'heure
				if((NEW.compensation_heure is null OR NEW.compensation_heure = false) and (NEW.heure_supplementaire is null OR NEW.heure_supplementaire = false)) then
					...
					update yvs_grh_presence set total_presence = total_ where id = OLD.presence and valide = false;
				else
					if(NEW.heure_supplementaire = true) then
						update yvs_grh_presence set total_heure_sup = total_heure_sup + NEW_somme_ - OLD_somme_ where id = OLD.presence and valide = false;
					end if;
					if(NEW.compensation_heure = true) then
						update yvs_grh_presence set total_heure_compensation = total_heure_compensation + NEW_somme_ - OLD_somme_ where id = OLD.presence and valide = false;
					end if;
				end if;
				...
				if(NEW.compensation_heure = false and NEW.heure_supplementaire = false) then
					...
					if(NEW.heure_supplementaire = true) then
						update yvs_grh_presence set total_heure_sup = total_heure_sup - NEW_somme_ where id = OLD.presence and valide = false;
					end if;
					if(NEW.compensation_heure = true) then
						update yvs_grh_presence set total_heure_compensation = total_heure_compensation - NEW_somme_ where id = OLD.presence and valide = false;
					end if;
				end if;
			end if;
