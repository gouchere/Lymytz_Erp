...
CREATE OR REPLACE FUNCTION et_balance_age_client(IN societe_ BIGINT, IN ecart_ INTEGER, IN colonne_ INTEGER, date_debut_ DATE, date_fin_ DATE)
...
BEGIN    
	...
	IF(_client_ IS NOT NULL)THEN
		...
		SELECT INTO avance_ COALESCE(SUM(p.montant), 0) FROM yvs_compta_caisse_piece_vente p INNER JOIN yvs_com_doc_ventes d ON p.vente = d.id
			WHERE p.statut_piece = 'P' AND d.client = client_;
		FOR facture_ IN SELECT d.id FROM yvs_com_doc_ventes d WHERE d.type_doc = 'FV' AND d.statut = 'V' AND d.client = client_
		LOOP
