DROP TRIGGER mut_action_reglement ON yvs_mut_reglement_mensualite;
...
CREATE TRIGGER mut_action_reglement_mensualite
  AFTER INSERT OR UPDATE OR DELETE
  ON yvs_mut_reglement_mensualite
  FOR EACH ROW
  EXECUTE PROCEDURE mut_action_reglement_mensualite();
...
BEGIN	
	...
	IF(credit_.montant <= payer_ AND credit_.etat = 'V')THEN
		UPDATE yvs_mut_credit SET etat = 'P', montant_verse = montant WHERE id = id_credit;
	ELSE
		UPDATE yvs_mut_credit SET montant_verse = payer_ WHERE id = id_credit;
	END IF;
...
CREATE TRIGGER mut_action_reglement_credit
  AFTER INSERT OR UPDATE OR DELETE
  ON yvs_mut_reglement_credit
  FOR EACH ROW
  EXECUTE PROCEDURE mut_action_reglement_credit();