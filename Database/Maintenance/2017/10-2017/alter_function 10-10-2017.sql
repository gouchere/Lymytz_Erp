...
BEGIN	
	...
	FOR conges_  IN SELECT c.id FROM yvs_grh_conge_emps c WHERE c.employe = conges_.employe AND c.statut IN('V','T','C') AND c.type_conge='CT' AND (c.date_fin between (conges_.date_debut - integer '1')::date and conges_.date_debut)
	LOOP	
...
BEGIN	
	...
	LOOP			
		...
		existe_ = FALSE;
		-- recherche du congé consécutif au congé en cours
		...
		IF(existe_ IS FALSE)THEN
			...
			date_debut = date_calcul + ((30* ((num_mois-1))||' day') :: interval) + ((num_mois-1)||' day') :: interval;
			
			--determination de la date de fin
			date_fin = date_calcul + ((30*(num_mois))||' day') :: interval;
