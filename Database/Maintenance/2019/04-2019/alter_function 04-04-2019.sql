...
begin 	
	...
	return (select compta_et_debit(agence_, societe_, valeur_, date_initial_, date_fin_, type_, general, table_tiers_));
end;
$BODY$
...
begin 	
	date_fin_ = date_debut_ - interval '1 day';	
	return (select compta_et_credit(agence_, societe_, valeur_, date_initial_, date_fin_, type_, general, table_tiers_));
end;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
...
-- Function: compta_clean_content_journal(bigint, bigint, date, date, boolean)
-- DROP FUNCTION compta_clean_content_journal(bigint, bigint, date, date, boolean);
CREATE OR REPLACE FUNCTION compta_clean_content_journal(IN societe_ bigint, IN agence_ bigint, IN date_debut_ date, IN date_fin_ date, IN delete_ boolean)
  RETURNS TABLE(id bigint, numero character varying, externe bigint, table_externe character varying) AS
$BODY$
declare 
	data_ record;

	add_ BOOLEAN DEFAULT FALSE;

	exist_ BIGINT;
	
	query_ character varying default 'SELECT c.id, c.num_ref, c.ref_externe, c.table_externe FROM yvs_compta_content_journal c INNER JOIN yvs_compta_pieces_comptable p ON c.piece = p.id INNER JOIN yvs_compta_journaux j ON p.journal = j.id INNER JOIN yvs_agences a ON j.agence = a.id WHERE a.societe = '||societe_;
begin 	
	DROP TABLE IF EXISTS table_clean_content_journal;
	CREATE TEMP TABLE IF NOT EXISTS table_clean_content_journal(_id bigint, _numero character varying, _externe bigint, _table_externe character varying); 
	DELETE FROM table_clean_content_journal;
	...
	LOOP
		add_ = false;
		IF(data_.table_externe = 'DOC_VENTE' OR data_.table_externe = 'AVOIR_VENTE')THEN
			SELECT INTO exist_ y.id FROM yvs_com_doc_ventes y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'JOURNAL_VENTE')THEN
			SELECT INTO exist_ y.id FROM yvs_com_entete_doc_vente y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'DOC_ACHAT' OR data_.table_externe = 'AVOIR_ACHAT')THEN
			SELECT INTO exist_ y.id FROM yvs_com_doc_achats y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'FRAIS_MISSION')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_caisse_piece_mission y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'PHASE_CAISSE_VENTE')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_phase_piece y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'PHASE_CAISSE_ACHAT')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_phase_piece_achat y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'PHASE_CAISSE_DIVERS')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_phase_piece_divers y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'PHASE_ACOMPTE_ACHAT')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_phase_acompte_achat y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'PHASE_ACOMPTE_VENTE')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_phase_acompte_vente y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'CAISSE_VENTE')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_caisse_piece_vente y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'CAISSE_ACHAT')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_caisse_piece_achat y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'CAISSE_DIVERS')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_caisse_piece_divers y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'ACOMPTE_VENTE')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_acompte_client y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'ACOMPTE_ACHAT')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_acompte_fournisseur y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'CREDIT_VENTE')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_credit_client y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'CREDIT_ACHAT')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_credit_fournisseur y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'CAISSE_CREDIT_VENTE')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_reglement_credit_client y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'CAISSE_CREDIT_ACHAT')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_reglement_credit_fournisseur y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'DOC_VIREMENT')THEN
			SELECT INTO exist_ y.id FROM yvs_compta_caisse_piece_virement y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		ELSIF(data_.table_externe = 'ORDRE_SALAIRE')THEN
			SELECT INTO exist_ y.id FROM yvs_grh_ordre_calcul_salaire y WHERE y.id = data_.ref_externe;
			IF(COALESCE(exist_, 0) < 1)THEN
				add_ = true;
			END IF;
		END IF;
		IF(add_)THEN
		    IF(delete_)THEN
				DELETE FROM yvs_compta_content_journal y WHERE y.id = data_.id;
			END IF;
			INSERT INTO table_clean_content_journal VALUES(data_.id, data_.num_ref, data_.ref_externe, data_.table_externe);
		END IF;
	END LOOP;
	IF(delete_)THEN
		DELETE FROM yvs_compta_pieces_comptable y WHERE y.id NOT IN (SELECT DISTINCT c.piece FROM yvs_compta_content_journal c);
	END IF;
	RETURN QUERY SELECT * FROM table_clean_content_journal ORDER BY _id, _numero;
END;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100
  ROWS 1000;
ALTER FUNCTION compta_clean_content_journal(bigint, bigint, date, date, boolean)
  OWNER TO postgres;
...
-- Function: compta_et_grand_livre(bigint, bigint, character varying, date, date, character varying, boolean)
		...
		LOOP
			IF(COALESCE(data_.id, 0) > 0)THEN
				IF(last_code_ != data_.code)THEN
					debit_si_ = (select compta_et_debit_initial(agence_, societe_, data_.id, data_.date_piece, type_, false, data_.table_tiers));
					credit_si_ = (select compta_et_credit_initial(agence_, societe_, data_.id, data_.date_piece, type_, false, data_.table_tiers));		
