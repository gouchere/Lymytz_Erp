...
BEGIN
	CASE tableexterne_
		...
		WHEN 'yvs_com_contenu_doc_stock' THEN	
			select into ligne_ d.type_doc, c.qualite, c.conditionnement, c.conditionnement_entree, c.lot_sortie as lot FROM yvs_com_contenu_doc_stock c inner join yvs_com_doc_stocks d on d.id=c.doc_stock WHERE c.id=idexterne_ limit 1;
		...
		WHEN 'yvs_prod_of_suivi_flux' THEN	
			select into ligne_ c.unite as conditionnement, null::bigint as qualite, null::bigint as lot  from yvs_prod_of_suivi_flux y inner join yvs_prod_flux_composant f on y.composant = f.id inner join yvs_prod_composant_of c on f.composant = c.id where y.id = idexterne_;
			if(mouvement_='S') then
		...
		WHEN 'yvs_prod_declaration_production' THEN	
			select into ligne_ conditionnement, null::bigint as qualite, null::bigint as lot from yvs_prod_declaration_production where id = idexterne_;
			operation_='Production';
		WHEN 'yvs_prod_contenu_conditionnement' THEN	
			select into ligne_ conditionnement, null::bigint as qualite, null::bigint as lot from yvs_prod_contenu_conditionnement where id = idexterne_;
			operation_='Conditionnement';
		WHEN 'yvs_prod_fiche_conditionnement' THEN	
			select into ligne_ unite_mesure as conditionnement, null::bigint as qualite, null::bigint as lot from yvs_prod_fiche_conditionnement f inner join yvs_prod_nomenclature n on f.nomenclature = n.id where f.id = idexterne_;
			operation_='Conditionnement';
		ELSE
			RETURN -1;
	END CASE;
	if(parent_ is not null)then
		if(tranche_ is not null and tranche_ > 0)then
			INSERT INTO yvs_base_mouvement_stock(quantite, date_doc, mouvement, article, supp, actif, id_externe, table_externe, description, depot, tranche, parent, cout_entree, cout_stock, date_mouvement, qualite, conditionnement, lot)
					VALUES (quantite_, date_, mouvement_, article_::bigint, false, true, idexterne_::bigint, tableexterne_, operation_, depot_::bigint, tranche_::bigint, parent_::bigint, coutentree_, cout_, current_timestamp, ligne_.qualite::bigint, ligne_.conditionnement::bigint, ligne_.lot::bigint);
		else
			INSERT INTO yvs_base_mouvement_stock(quantite, date_doc, mouvement, article, supp, actif, id_externe, table_externe, description, depot, parent, cout_entree, cout_stock, date_mouvement, qualite, conditionnement, lot)
					VALUES (quantite_, date_, mouvement_, article_::bigint, false, true, idexterne_::bigint, tableexterne_, operation_, depot_::bigint, parent_::bigint, coutentree_, cout_, current_timestamp, ligne_.qualite::bigint, ligne_.conditionnement::bigint, ligne_.lot::bigint);
		end if;
	else
		if(tranche_ is not null and tranche_ > 0)then
			INSERT INTO yvs_base_mouvement_stock(quantite, date_doc, mouvement, article, supp, actif, id_externe, table_externe, description, depot, tranche, cout_entree, cout_stock, date_mouvement, qualite, conditionnement, lot)
					VALUES (quantite_, date_, mouvement_, article_::bigint, false, true, idexterne_::bigint, tableexterne_, operation_, depot_::bigint, tranche_::bigint, coutentree_, cout_, current_timestamp, ligne_.qualite::bigint, ligne_.conditionnement::bigint, ligne_.lot::bigint);
		else
			INSERT INTO yvs_base_mouvement_stock(quantite, date_doc, mouvement, article, supp, actif, id_externe, table_externe, description, depot, cout_entree, cout_stock, date_mouvement, qualite, conditionnement, lot)
					VALUES (quantite_, date_, mouvement_, article_::bigint, false, true, idexterne_::bigint, tableexterne_, operation_, depot_::bigint, coutentree_, cout_, current_timestamp, ligne_.qualite::bigint, ligne_.conditionnement::bigint, ligne_.lot::bigint);
		end if;
	end if;
	return true;
END;$BODY$
...
-- DROP FUNCTION yvs_compta_content_analytique(bigint, bigint, bigint, bigint, bigint, double precision, character varying, character varying, boolean, boolean);
...
BEGIN   
	...
	IF((valeur_ IS NOT NULL AND valeur_ > 0) AND (compte_ IS NOT NULL AND compte_ > 0))THEN
		...
		ELSIF(table_ = 'FRAIS_MISSION')THEN
			query_ = 'SELECT DISTINCT ap.centre AS centre, (('||valeur_||') *(COALESCE(ap.coefficient, 1)/100)) AS montant, ap.coefficient AS coeficient FROM yvs_compta_centre_mission ap 
				INNER JOIN yvs_grh_missions mi ON ap.mission = mi.id INNER JOIN yvs_grh_objets_mission om ON mi.objet_mission = om.id
				WHERE (COALESCE(ap.coefficient,0)!=0) AND om.compte_charge = '||compte_||'';
		ELSE
...
-- Function: import_article(bigint, character varying, integer, character varying, character varying, character varying)
-- DROP FUNCTION import_article(bigint, character varying, integer, character varying, character varying, character varying);
CREATE OR REPLACE FUNCTION import_article(societe_ bigint, serveur character varying, port integer, database character varying, users character varying, password character varying)
  RETURNS boolean AS
$BODY$
DECLARE
    articles_ RECORD;
...
BEGIN
	FOR classes_  IN SELECT * FROM dblink('host='||serveur||' dbname='||database||' user='||users||' password='||password,'SELECT * FROM classestat ')
	...
	LOOP
		...
		END IF;
	END LOOP;
	FOR unites_  IN SELECT * FROM dblink('host='||serveur||' dbname='||database||' user='||users||' password='||password,'SELECT * FROM yvs_unite_mesure ')
		AS t(id integer, reference character varying, libelle character varying, type character varying)
	LOOP
		...
		END IF;
	END LOOP;
	FOR familles_  IN SELECT * FROM dblink('host='||serveur||' dbname='||database||' user='||users||' password='||password,'SELECT * FROM famillearticles ')
	...
	LOOP
		...
		END IF;
		FOR articles_  IN SELECT * FROM dblink('host='||serveur||' dbname='||database||' user='||users||' password='||password,'SELECT * FROM articles WHERE reffamille = '||QUOTE_LITERAL(familles_.reffamille))
			AS t(refart character varying, categorie character varying, changeprix boolean, classe character varying, codebarre character varying, commentaire character varying, conditionnement character varying, 
				datesave timestamp without time zone, defnorme boolean, designation character varying, modeconso character, new_ref character varying, norme character varying, photos character varying, poidnet double precision, 
				pua double precision, puv double precision, remise double precision, sommeil boolean, suivienstock boolean, taill_du_lot double precision, typepv character varying, unite character varying, 
				unitepoids character varying, visibleensynthese boolean, code_acces bigint, code_sortie bigint, reffamille character varying, classe2 character varying, appliquer_remise boolean)
		LOOP
			...
			IF(COALESCE(article_, 0) = 0)THEN
				...
				INSERT INTO yvs_base_articles(change_prix, description, def_norme, designation, mode_conso, norme, photo_1, masse_net, prix_min, pua, puv, ref_art, remise, 
				    ...
				    VALUES (articles_.changeprix, COALESCE(articles_.commentaire, ''), articles_.defnorme, articles_.designation, articles_.modeconso, articles_.defnorme, articles_.photos, articles_.poidnet, articles_.puv, articles_.pua, articles_.puv, articles_.refart, articles_.remise,
					    articles_.suivienstock, articles_.visibleensynthese, null, articles_.classe, 0, null, 'FIFO', true, null, null, null, articles_.categorie, 
			...
			END IF;
			FOR conditionnements_  IN SELECT * FROM dblink('host='||serveur||' dbname='||database||' user='||users||' password='||password,'SELECT c.*, u.reference FROM conditionnement c INNER JOIN yvs_unite_mesure u ON c.unite = u.id WHERE article = '||QUOTE_LITERAL(articles_.refart))
				AS t(id integer, unite integer, article character varying, by_vente boolean, by_achat boolean, by_prod boolean, pua double precision, puv double precision, remise double precision, conditionnement character varying, reference character varying)
			LOOP
...
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION import_article(bigint, character varying, integer, character varying, character varying, character varying)
  OWNER TO postgres;
...
BEGIN
	...
	RAISE NOTICE 'type_doc : %',NEW.type_doc;
	RAISE NOTICE 'statut : %',NEW.statut;
	if(NEW.type_doc = 'FT' and (NEW.statut = 'U' or NEW.statut = 'V')) then
		...
		loop
			...
			if(NEW.destination is not null and NEW.statut = 'V')then  --traitement de l'entrée dans le dépôt de destination
				...
				result = (select valorisation_stock(cont_.article, cont_.conditionnement_entree, NEW.destination, trancheD_, cont_.quantite_entree, cont_.prix_entree, 'yvs_com_contenu_doc_stock', cont_.id, 'E', NEW.date_reception));
			end if;
		end loop;
...
BEGIN
	...
	if(doc_.statut = 'V' or doc_.statut = 'U') then
		if(doc_.type_doc = 'FT') then
			...
			if(doc_.destination is not null and (doc_.statut = 'V' or (NEW.statut = 'V' and doc_.statut = 'U')))then
				result = (select valorisation_stock(NEW.article,NEW.conditionnement_entree, doc_.destination, trancheD_, NEW.quantite_entree, NEW.prix_entree, ''||TG_TABLE_NAME||'', NEW.id, 'E', doc_.date_reception));
			end if;
...
BEGIN
	...
	if(doc_.statut = 'V' or doc_.statut = 'U') then
		if(doc_.type_doc = 'FT') then	
			if(doc_.source is not null and (doc_.statut = 'U' or doc_.statut = 'V'))then
				select into mouv_ id from yvs_base_mouvement_stock WHERE id_externe = NEW .id and table_externe = TG_TABLE_NAME and depot = doc_.source and mouvement = 'S';				
				if(mouv_ is not null)then
					if(arts_.methode_val = 'FIFO')then
						delete from yvs_base_mouvement_stock where id = mouv_;
						result = (select valorisation_stock(NEW.article,NEW.conditionnement, doc_.source, trancheS_, NEW.quantite, NEW.prix, ''||TG_TABLE_NAME||'', NEW.id, 'S', doc_.date_doc));
					else
						UPDATE yvs_base_mouvement_stock SET quantite = NEW.quantite, cout_entree = NEW.prix, conditionnement = NEW.conditionnement, lot = NEW.lot_sortie, date_doc=doc_.date_doc
														WHERE id = mouv_;
					end if;
				else
					result = (select valorisation_stock(NEW.article,NEW.conditionnement, doc_.source, trancheS_, NEW.quantite, NEW.prix, ''||TG_TABLE_NAME||'', NEW.id, 'S', doc_.date_doc));
				...
				if(doc_.destination is not null)then			
					...
					if(mouv_ is not null)then
						if(arts_.methode_val = 'FIFO')then
							...
							UPDATE yvs_base_mouvement_stock SET quantite = NEW.quantite_entree, cout_entree = NEW.prix_entree, conditionnement = NEW.conditionnement_entree, lot = NEW.lot_sortie, date_doc = doc_.date_reception
															WHERE id = mouv_;
						end if;
					else
						result = (select valorisation_stock(NEW.article,NEW.conditionnement_entree, doc_.destination, trancheD_, NEW.quantite_entree, NEW.prix_entree, ''||TG_TABLE_NAME||'', NEW.id, 'E', doc_.date_reception));
			...
			IF(doc_.destination is not null)then
				...
				IF(mouv_ is not null)THEN
						IF(arts_.methode_val = 'FIFO')then
							...
							result = (select valorisation_stock(NEW.article,NEW.conditionnement_entree, doc_.destination, trancheD_, NEW.quantite, NEW.prix, ''||TG_TABLE_NAME||'', NEW.id, 'E', doc_.date_doc));
						ELSE
							UPDATE yvs_base_mouvement_stock SET quantite = NEW.quantite, cout_entree = NEW.prix, conditionnement = NEW.conditionnement_entree, lot = NEW.lot_sortie , date_doc=doc_.date_doc
															WHERE id = mouv_;
						END IF;
				ELSE
					result = (select valorisation_stock(NEW.article,NEW.conditionnement_entree, doc_.destination, trancheD_, NEW.quantite, NEW.prix, ''||TG_TABLE_NAME||'', NEW.id, 'E', doc_.date_doc));
			...
			if(doc_.source is not null)then
				...
				if(mouv_ is not null)then
					if(arts_.methode_val = 'FIFO')then
						...
						UPDATE yvs_base_mouvement_stock SET quantite = NEW.quantite_entree, cout_entree = NEW.prix_entree, conditionnement = NEW.conditionnement_entree, lot = NEW.lot_entree, date_doc=doc_.date_doc 
...
-- Function: com_et_ration(bigint, bigint)
...
-- 	DROP TABLE IF EXISTS table_ration;
	...
	FOR ligne_ IN SELECT p.id, a.ref_art, a.designation, p.quantite, u.reference, t.code_tiers, CONCAT(t.nom, ' ', t.prenom) AS nom, p.periode, y.nbr_jr_usine, p.article, p.personnel, p.conditionnement, p.proportionnel, p.actif, COALESCE(y.date_fiche, e.date_debut) AS date_fiche
		FROM yvs_com_param_ration p INNER JOIN yvs_com_periode_ration e ON p.date_prise_effet <= e.fin INNER JOIN yvs_base_articles a ON p.article = a.id INNER JOIN yvs_base_conditionnement c ON p.conditionnement = c.id
		INNER JOIN yvs_base_tiers t ON p.personnel = t.id INNER JOIN yvs_base_unite_mesure u ON c.unite = u.id INNER JOIN yvs_base_famille_article f ON a.famille = f.id LEFT JOIN yvs_com_doc_ration y ON y.periode = e.id
		WHERE y.id = id_ AND f.societe = societe_ ORDER BY t.nom, t.prenom, t.code_tiers
	LOOP
		SELECT INTO suspendu_ s.id FROM yvs_com_param_ration_suspension s WHERE s.personnel = ligne_.id AND ligne_.date_fiche NOT BETWEEN s.debut_suspension AND s.fin_suspension;
		IF(COALESCE(suspendu_, 0) = 0)THEN
