...
-- Function: update_tiers_doc_divers_()
-- DROP FUNCTION update_tiers_doc_divers_();
CREATE OR REPLACE FUNCTION update_tiers_doc_divers_()
  RETURNS boolean AS
$BODY$   
DECLARE 
	contenu_ RECORD;
...
BEGIN
	FOR contenu_ IN SELECT DISTINCT p.id from yvs_compta_pieces_comptable p INNER JOIN yvs_compta_content_journal c ON c.piece = p.id
		WHERE c.compte_general IN (63, 81, 485, 513) AND c.table_externe IN ('ACOMPTE_VENTE', 'CAISSE_VENTE')
		AND (SELECT COUNT(y.id) FROM yvs_compta_content_journal y WHERE y.piece = p.id AND COALESCE(y.compte_tiers, 0) > 0) = 2
		ORDER BY p.id
	LOOP
		count_ = count_ + 1;
		RAISE NOTICE '% : contenu_.id : %', count_,contenu_.id;
		UPDATE yvs_compta_content_journal SET compte_tiers = NULL WHERE piece = contenu_.id;
	END LOOP;
	count_ = 0;
	FOR contenu_ IN SELECT id, lettrage FROM yvs_compta_content_journal WHERE COALESCE(lettrage, '') != ''
	LOOP	
		SELECT INTO id_ id FROM yvs_compta_content_journal WHERE COALESCE(lettrage, '') = contenu_.lettrage AND id != contenu_.id LIMIT 1;
		IF(COALESCE(id_, 0) < 1)THEN
			count_ = count_ + 1;
			RAISE NOTICE '% : contenu_.id : %', count_,contenu_.id;
			UPDATE yvs_compta_content_journal SET lettrage = null WHERE id = contenu_.id;
		END IF;
	END LOOP;
	RETURN TRUE;
END;$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION update_tiers_doc_divers_()
  OWNER TO postgres;
...
CREATE OR REPLACE FUNCTION valorisation_stock_by_cmp1(article_ bigint, conditionnement_ bigint, depot_ bigint, tranche_ bigint, quantite_ double precision, cout_ double precision, tableexterne_ character varying, idexterne_ bigint, date_ date)
  RETURNS boolean AS
$BODY$
DECLARE 
	entree_ record;  
...
BEGIN
	...
	--	select into entree_ quantite, cout_stock as cout from yvs_base_mouvement_stock where mouvement = 'E' and article = article_ AND conditionnement=conditionnement_ and depot = depot_ and tranche = tranche_ order by date_doc DESC, id desc limit 1;
	--else
		select into entree_ quantite, cout_stock as cout from yvs_base_mouvement_stock WHERE calcul_pr IS TRUE AND mouvement = 'E' AND article = article_ AND conditionnement=conditionnement_ AND depot = depot_ ORDER BY date_doc DESC, id desc limit 1;
	--	end if;
	--calcule le stock
		stock_=get_stock_reel(article_,0,depot_,0,0,date_,conditionnement_,0);
	...
	-- Calcul du cout de stockage
	prix_arr_ = stock_ + quantite_;
	if(prix_arr_ <= 0)then
		prix_arr_ = 1;
	end if;
	new_cout = (((stock_ * entree_.cout) + (quantite_ * cout_)) / (prix_arr_));
...
-- Trigger: delete_ on yvs_compta_content_journal
-- DROP TRIGGER delete_ ON yvs_compta_content_journal;
CREATE TRIGGER delete_
  AFTER DELETE
  ON yvs_compta_content_journal
  FOR EACH ROW
  EXECUTE PROCEDURE delete_content_journal();
			...
			IF(data_.numero_piece IS NOT NULL AND data_.numero_piece NOT IN ('', ' ') AND data_.type_reglement != 'SALAIRE')THEN	
				...
				ELSE
					...
					IF(data_.type_reglement = 'COMPENSATION')THEN					
						...
						END IF;
					ELSE				
						SELECT INTO ligne_ COALESCE(a.nature, 'A') AS nature, CHAR_LENGTH(COALESCE(a.nature, 'A')) as count FROM yvs_compta_notif_reglement_vente n INNER JOIN yvs_compta_acompte_client a ON n.acompte = a.id WHERE n.piece_vente = element_ LIMIT 1;
			...
			IF(data_.numero_piece IS NOT NULL AND data_.numero_piece NOT IN ('', ' '))THEN	
				...
				ELSE
					...
					IF(data_.achat IS NULL OR data_.achat < 1)THEN
						_error_ = 'ce reglement n''est associé à aucune facture';
					ELSE
						SELECT INTO ligne_ COALESCE(a.nature, 'A') AS nature, CHAR_LENGTH(COALESCE(a.nature, 'A')) as count FROM yvs_compta_notif_reglement_achat n INNER JOIN yvs_compta_acompte_fournisseur a ON n.acompte = a.id WHERE n.piece_achat = element_ LIMIT 1;
						_libelle_ = _libelle_||data_.num_doc;
						IF(data_.fournisseur IS NULL OR data_.fournisseur < 1)THEN
							_error_ = 'ce reglement est rattaché à une facture qui n''a pas de fournisseur';
						ELSE
