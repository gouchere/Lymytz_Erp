...
BEGIN    
	...
	loop
		...
		LOOP			
			...
			END IF;
			SELECT INTO depot_ ad.depot FROM yvs_base_article_depot ad WHERE ad.article=_article.id AND ad.default_pr IS true limit 1;
			SELECT INTO pr_ get_pr(_article.id, COALESCE(depot_,0),0,dates_.date_fin,_article.unite);
			--SELECT INTO pr_ y.cout_stock FROM yvs_base_mouvement_stock y WHERE y.conditionnement = _article.unite AND y.date_doc BETWEEN dates_.date_debut AND dates_.date_fin AND y.mouvement='E' AND y.cout_stock>0
		    --														 AND y.table_externe IN ('yvs_com_contenu_doc_achat','yvs_prod_declaration_production') ORDER BY y.date_doc DESC limit 1;
			IF(pr_ IS NULL)THEN
				pr_ = 0;
			END IF;
