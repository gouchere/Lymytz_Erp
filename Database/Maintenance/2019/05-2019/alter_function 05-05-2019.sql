				...
				if(doc_.destination is not null)then			
					...
					if(mouv_ is not null)then
						if(arts_.methode_val = 'FIFO')then
							...
							UPDATE yvs_base_mouvement_stock SET quantite = NEW.quantite_entree, cout_entree = NEW.prix_entree, article = NEW.article, conditionnement = NEW.conditionnement_entree, lot = NEW.lot_sortie, calcul_pr = NEW.calcul_pr, date_doc = NEW.date_reception WHERE id = mouv_;
	...
	IF(COALESCE(societe_, 0) > 0)THEN
		...
		LOOP
			SELECT INTO last_date_ e.date_entete FROM yvs_com_entete_doc_vente e INNER JOIN yvs_com_doc_ventes d ON d.entete_doc = e.id INNER JOIN yvs_com_contenu_doc_vente c ON c.doc_vente = d.id
				WHERE c.article = data_.id ORDER BY e.date_entete DESC LIMIT 1;
...
-- Function: base_legende_classe_statistique(bigint, integer, integer, character varying)
DROP FUNCTION base_legende_classe_statistique(bigint, integer, integer);
CREATE OR REPLACE FUNCTION base_legende_classe_statistique(IN societe_ bigint, IN offset_ integer, IN limit_ integer, IN type_ character varying)
...
BEGIN
	...
	END IF;
	IF(COALESCE(type_, '') = 'F')THEN
		FOR result_ IN SELECT y.id, y.designation FROM yvs_base_famille_article y WHERE y.societe = societe_ OFFSET offset_ LIMIT limit_
		LOOP
...
END;$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION base_legende_classe_statistique(bigint, integer, integer, character varying)
  OWNER TO postgres;
