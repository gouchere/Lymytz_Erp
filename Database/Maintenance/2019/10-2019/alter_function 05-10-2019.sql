...
begin 	
	...
	if((header_ is not null and header_ !='') and (agence_ is not null and agence_ !=''))then
		...
		loop
			...
			inner join yvs_grh_bulletins b on d.bulletin = b.id INNER JOIN yvs_grh_header_bulletin h on h.bulletin = b.id inner join yvs_grh_contrat_emps c on b.contrat = c.id
			inner join yvs_grh_employes y on c.employe = y.id
			where b.entete::character varying in (select val from regexp_split_to_table(header_,',') val) and e.id = element_.id 
