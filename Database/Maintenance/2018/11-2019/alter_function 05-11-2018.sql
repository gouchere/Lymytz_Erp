	...
	IF(table_ IS NOT NULL AND table_ NOT IN ('', ' '))THEN
		...
		ELSIF(table_ = 'ACOMPTE_VENTE')THEN
			SELECT INTO data_ a.date_acompte, a.num_refrence, a.montant, m.societe, c.compte, c.tiers FROM yvs_compta_acompte_client a 
				INNER JOIN yvs_base_mode_reglement m ON a.model = m.id INNER JOIN yvs_com_client c ON a.client = c.id WHERE a.id = element_;
		...
		ELSIF(table_ = 'ACOMPTE_ACHAT')THEN
			SELECT INTO data_ a.date_acompte, a.num_refrence, a.montant, m.societe, c.compte, c.tiers FROM yvs_compta_acompte_fournisseur a 
				INNER JOIN yvs_base_mode_reglement m ON a.model = m.id INNER JOIN yvs_base_fournisseur c ON a.fournisseur = c.id WHERE a.id = element_;
			IF(data_.num_refrence IS NOT NULL AND data_.num_refrence NOT IN ('', ' '))THEN
				_jour_ = to_char(data_.date_acompte ,'MM')::integer;
				_num_piece_ = data_.num_refrence;	
				_echeance_ = data_.date_acompte;
		...
		ELSIF(table_ = 'CREDIT_VENTE')THEN
			SELECT INTO data_ a.date_credit, a.num_reference, a.montant, m.societe, c.compte, c.tiers, t.compte as compte_general FROM yvs_compta_credit_client a 
		...
		ELSIF(table_ = 'CREDIT_ACHAT')THEN
			SELECT INTO data_ a.date_credit, a.num_reference, a.montant, m.societe, c.compte, c.tiers, t.compte as compte_general FROM yvs_compta_credit_fournisseur a 
