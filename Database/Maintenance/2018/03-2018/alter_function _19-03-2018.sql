...
-- 	DROP TABLE IF EXISTS table_decoupage_interval_date;
	...
	loop
		if(periode_ = 'A')then
			...
			intitule_ = to_char(date_action_ ,'dd');
		end if;
			...
			loop
				...
				if(agence_ is null or agence_ < 1)then
					select into total_ coalesce(sum(c.prix_total), 0) as ttc, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
						inner join yvs_com_creneau_horaire_users u on e.creneau = u.id where d.type_doc = 'FV' and e.date_entete  BETWEEN dates_.date_debut AND dates_.date_fin and d.client = client_ and d.statut = 'V';
					...
					select into total_ coalesce(sum(c.prix_total), 0) as ttc, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
						...
						where d.type_doc = 'FV' and p.agence = agence_ and e.date_entete  BETWEEN dates_.date_debut AND dates_.date_fin and d.client = client_ and d.statut = 'V';
				end if;
		...
		loop
			...
			if(agence_ is null or agence_ < 1)then
				for  data_ in select c.article, a.ref_art, a.designation ,coalesce(sum(c.prix_total), 0) as total, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_base_articles a on c.article = a.id inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
					inner join yvs_com_creneau_horaire_users u on e.creneau = u.id where d.type_doc = 'FV' and e.date_entete between dates_.date_debut AND dates_.date_fin and d.client = client_ and d.statut = 'V' group by c.article, a.ref_art, a.designation order by a.ref_art
				loop
				...
				for data_ in select c.article, a.ref_art, a.designation ,coalesce(sum(c.prix_total), 0) as total, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_base_articles a on c.article = a.id inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
					inner join yvs_com_creneau_horaire_users u on e.creneau = u.id inner join yvs_com_creneau_point o on u.creneau_point = o.id inner join yvs_base_point_vente p on o.point = p.id
					where d.type_doc = 'FV' and p.agence = agence_ and e.date_entete between dates_.date_debut AND dates_.date_fin and d.client = client_ and d.statut = 'V' group by c.article, a.ref_art, a.designation order by a.ref_art
				loop
					select into prec_ ttc as total from client_article_by_jour where art = data_.article order by rang desc limit 1;
		...
		loop
			...
			if(agence_ is null or agence_ < 1)then
				for  data_ in select c.article, a.ref_art, a.designation, coalesce(sum(c.prix_total), 0) as total, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_base_articles a on c.article = a.id inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
					...
					where d.type_doc = 'FV' and e.date_entete between dates_.date_debut AND dates_.date_fin and o.point = point_ and d.statut = 'V' group by c.article, a.ref_art, a.designation order by a.ref_art
				loop
				...
				for data_ in select c.article, a.ref_art, a.designation, coalesce(sum(c.prix_total), 0) as total, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_base_articles a on c.article = a.id inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
					inner join yvs_com_creneau_horaire_users u on e.creneau = u.id inner join yvs_com_creneau_point o on u.creneau_point = o.id inner join yvs_base_point_vente p on o.point = p.id
					where d.type_doc = 'FV' and p.agence = agence_ and e.date_entete between dates_.date_debut AND dates_.date_fin and o.point = point_ and d.statut = 'V' group by c.article, a.ref_art, a.designation order by a.ref_art
				loop
					select into prec_ ttc as total from point_vente_article_by_jour where art = data_.article order by rang desc limit 1;
...
BEGIN    
	...
	if(_vendeur_ is not null)then
		...
		loop
			...
			if(agence_ is null or agence_ < 1)then
				for  data_ in select c.article, a.ref_art, a.designation, coalesce(sum(c.prix_total), 0) as total, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_base_articles a on c.article = a.id inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
					inner join yvs_com_creneau_horaire_users u on e.creneau = u.id where d.type_doc = 'FV' and e.date_entete between dates_.date_debut AND dates_.date_fin and u.users = vendeur_ and d.statut = 'V' group by c.article, a.ref_art, a.designation order by a.ref_art
				loop
				...
				for data_ in select c.article, a.ref_art, a.designation, coalesce(sum(c.prix_total), 0) as total, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_base_articles a on c.article = a.id inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
					inner join yvs_com_creneau_horaire_users u on e.creneau = u.id inner join yvs_com_creneau_point o on u.creneau_point = o.id inner join yvs_base_point_vente p on o.point = p.id
					where d.type_doc = 'FV' and p.agence = agence_ and e.date_entete between dates_.date_debut AND dates_.date_fin and u.users = vendeur_ and d.statut = 'V' group by c.article, a.ref_art, a.designation order by a.ref_art
				loop
					select into prec_ ttc as total from vendeur_article_by_jour where art = data_.article order by rang desc limit 1;
		...
		loop
			...
			IF(agence_ IS NULL OR agence_ < 1)THEN
				SELECT INTO data_ COALESCE(sum(c.prix_total), 0) AS total, COALESCE(sum(c.quantite), 0) AS qte FROM yvs_com_contenu_doc_vente c 
					...
					WHERE d.type_doc = 'FV' AND e.date_entete between dates_.date_debut AND dates_.date_fin AND c.article = _article.id AND d.statut = 'V';				
			else
				SELECT INTO data_ COALESCE(sum(c.prix_total), 0) AS total, COALESCE(sum(c.quantite), 0) AS qte FROM yvs_com_contenu_doc_vente c 
					INNER JOIN yvs_base_articles a ON c.article = a.id INNER JOIN yvs_com_doc_ventes d ON c.doc_vente = d.id 
					INNER JOIN yvs_com_entete_doc_vente e ON e.id = d.entete_doc INNER JOIN yvs_com_creneau_horaire_users u ON e.creneau = u.id 
					INNER JOIN yvs_com_creneau_point o ON u.creneau_point = o.id INNER JOIN yvs_base_point_vente p ON o.point = p.id
					WHERE d.type_doc = 'FV' AND p.agence = agence_ AND e.date_entete between dates_.date_debut AND dates_.date_fin AND c.article = _article.id AND d.statut = 'V';
			END IF;
	...
	if(_point_ is not null)then
		...
		loop
			...
			if(agence_ is null or agence_ < 1)then
				select into total_ coalesce(sum(c.prix_total), 0) as ttc, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
					inner join yvs_com_creneau_horaire_users u on e.creneau = u.id inner join yvs_com_creneau_point o on u.creneau_point = o.id
					where d.type_doc = 'FV' and e.date_entete between dates_.date_debut AND dates_.date_fin and o.point = point_ and d.statut = 'V';
				
			else
				select into total_ coalesce(sum(c.prix_total), 0) as ttc, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
					inner join yvs_com_creneau_horaire_users u on e.creneau = u.id inner join yvs_com_creneau_point o on u.creneau_point = o.id inner join yvs_base_point_vente p on o.point = p.id
					where d.type_doc = 'FV' and p.agence = agence_ and e.date_entete between dates_.date_debut AND dates_.date_fin and o.point = point_ and d.statut = 'V';
			end if;
...
BEGIN    
	...
	if(_vendeur_ is not null)then
		...
		loop
			...
			if(agence_ is null or agence_ < 1)then
				select into total_ coalesce(sum(c.prix_total), 0) as ttc, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
					inner join yvs_com_creneau_horaire_users u on e.creneau = u.id where d.type_doc = 'FV' and e.date_entete between dates_.date_debut AND dates_.date_fin and u.users = vendeur_ and d.statut = 'V';
				
			else
				select into total_ coalesce(sum(c.prix_total), 0) as ttc, coalesce(sum(c.quantite), 0) as qte from yvs_com_contenu_doc_vente c inner join yvs_com_doc_ventes d on c.doc_vente = d.id inner join yvs_com_entete_doc_vente e on e.id = d.entete_doc
					inner join yvs_com_creneau_horaire_users u on e.creneau = u.id inner join yvs_com_creneau_point o on u.creneau_point = o.id inner join yvs_base_point_vente p on o.point = p.id
					where d.type_doc = 'FV' and p.agence = agence_ and e.date_entete between dates_.date_debut AND dates_.date_fin and u.users = vendeur_ and d.statut = 'V';
			end if;
		...
		loop
			...
			if(element_.retenue is false)then
				select into valeur_ coalesce(d.montant_payer, 0) from yvs_grh_detail_bulletin d inner join yvs_grh_bulletins b on d.bulletin = b.id 
					...
					where d.element_salaire = element_.id and c.employe = employe_ and o.debut_mois >= dates_.date_debut and o.fin_mois <= dates_.date_fin;
			else
			...
			LOOP
				...
				loop
					...
					IF(agence_ > 0)THEN
						...
						WHERE m.objet_mission = element_.id AND m.actif IS TRUE AND m.statut_mission IN ('V', 'C', 'R') AND e.agence = agence_ AND m.date_debut >= dates_.date_debut AND m.date_fin <= dates_.date_fin;
					ELSE
						SELECT INTO valeur_ SUM(COALESCE(y.montant, 0)) FROM yvs_grh_frais_mission y INNER JOIN yvs_grh_missions m ON y.mission = m.id INNER JOIN yvs_grh_employes e ON m.employe = e.id
						WHERE m.objet_mission = element_.id AND m.actif IS TRUE AND m.statut_mission IN ('V', 'C', 'R') AND m.date_debut >= dates_.date_debut AND m.date_fin <= dates_.date_fin;
					END IF;
		...
		ELSIF(type_ = 'L')THEN
			...
			LOOP
				...
				loop
					...
					IF(agence_ > 0)THEN
						SELECT INTO valeur_ SUM(COALESCE(y.montant, 0)) FROM yvs_grh_frais_mission y INNER JOIN yvs_grh_missions m ON y.mission = m.id INNER JOIN yvs_grh_employes e ON m.employe = e.id
						WHERE m.lieu = element_.id AND m.actif IS TRUE AND m.statut_mission IN ('V', 'C', 'R') AND e.agence = agence_ AND m.date_debut >= dates_.date_debut AND m.date_fin <= dates_.date_fin;
					ELSE
						SELECT INTO valeur_ SUM(COALESCE(y.montant, 0)) FROM yvs_grh_frais_mission y INNER JOIN yvs_grh_missions m ON y.mission = m.id INNER JOIN yvs_grh_employes e ON m.employe = e.id
						WHERE m.lieu = element_.id AND m.actif IS TRUE AND m.statut_mission IN ('V', 'C', 'R') AND m.date_debut >= dates_.date_debut AND m.date_fin <= dates_.date_fin;
					END IF;
					valeur_ = COALESCE(valeur_, 0);
					somme_ = somme_ + valeur_;
					i = i + 1;
		...
		ELSE
			...
			LOOP
				...
				loop
					...
					IF(agence_ > 0)THEN
						SELECT INTO valeur_ SUM(COALESCE(y.montant, 0)) FROM yvs_grh_frais_mission y INNER JOIN yvs_grh_missions m ON y.mission = m.id INNER JOIN yvs_grh_employes e ON m.employe = e.id
						WHERE m.employe = element_.id AND m.actif IS TRUE AND m.statut_mission IN ('V', 'C', 'R') AND e.agence = agence_ AND m.date_debut >= dates_.date_debut AND m.date_fin <= dates_.date_fin;
					ELSE
						SELECT INTO valeur_ SUM(COALESCE(y.montant, 0)) FROM yvs_grh_frais_mission y INNER JOIN yvs_grh_missions m ON y.mission = m.id INNER JOIN yvs_grh_employes e ON m.employe = e.id
						WHERE m.employe = element_.id AND m.actif IS TRUE AND m.statut_mission IN ('V', 'C', 'R') AND m.date_debut >= dates_.date_debut AND m.date_fin <= dates_.date_fin;
					END IF;
					valeur_ = COALESCE(valeur_, 0);
					somme_ = somme_ + valeur_;
					i = i + 1;
	...
	IF(_query_ IS NOT NULL AND _query_ != '')THEN
		...
		LOOP
			...
			loop
				...
				FOR _data_ IN SELECT * FROM grh_presence_durees(_employe_.id, _employe_.agence, societe_, dates_.date_debut, dates_.date_fin)
				LOOP
				...
				WHERE c.actif AND s.visible_bulletin = true AND y.now_visible = true AND c.employe = _employe_.id AND 
					...
					AND ((o.debut_mois BETWEEN dates_.date_debut AND dates_.date_fin) AND (o.fin_mois BETWEEN dates_.date_debut AND dates_.date_fin));
