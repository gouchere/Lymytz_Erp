CREATE OR REPLACE FUNCTION public.compta_et_debit_credit_initial(
    agence_ bigint, 
    societe_ bigint, 
    valeur_ bigint, 
    date_fin_ date, 
...
BEGIN 	
    ...
    WHERE date_fin_ BETWEEN date_debut AND date_fin
    LIMIT 1;
    
    -- Si aucune date trouvée, utiliser une valeur par défaut
    date_initial_ := COALESCE(date_initial_, '2000-01-01'::DATE);
    ...
    data_analytique AS (
        ...
        WHERE type_ = 'A'
            AND COALESCE(o.report, FALSE) IS TRUE
            AND o.date_piece BETWEEN date_initial_ AND date_fin_
    ...
    data_comptable AS (
        ...
        WHERE type_ != 'A'
            AND COALESCE(c.report, FALSE) IS TRUE
            AND c.date_piece BETWEEN date_initial_ AND date_fin_
